-- SPDX-License-Identifier: BSD-3-Clause
-- Copyright (c) 2025 curtissimo, llc. All Rights Reserved.


module Material.Icons.Directory.V exposing
    ( lookup
    , vacuum
    , vacuumOutline
    , valve
    , valveClosed
    , valveOpen
    , vanPassenger
    , vanUtility
    , vanish
    , vanishQuarter
    , vanityLight
    , variable
    , variableBox
    , vectorArrangeAbove
    , vectorArrangeBelow
    , vectorBezier
    , vectorCircle
    , vectorCircleVariant
    , vectorCombine
    , vectorCurve
    , vectorDifference
    , vectorDifferenceAb
    , vectorDifferenceBa
    , vectorEllipse
    , vectorIntersection
    , vectorLine
    , vectorLink
    , vectorPoint
    , vectorPointEdit
    , vectorPointMinus
    , vectorPointPlus
    , vectorPointSelect
    , vectorPolygon
    , vectorPolygonVariant
    , vectorPolyline
    , vectorPolylineEdit
    , vectorPolylineMinus
    , vectorPolylinePlus
    , vectorPolylineRemove
    , vectorRadius
    , vectorRectangle
    , vectorSelection
    , vectorSquare
    , vectorSquareClose
    , vectorSquareEdit
    , vectorSquareMinus
    , vectorSquareOpen
    , vectorSquarePlus
    , vectorSquareRemove
    , vectorTriangle
    , vectorUnion
    , vhs
    , vibrate
    , vibrateOff
    , video
    , video2d
    , video3d
    , video3dOff
    , video3dVariant
    , video4kBox
    , videoAccount
    , videoBox
    , videoBoxOff
    , videoCheck
    , videoCheckOutline
    , videoHighDefinition
    , videoImage
    , videoInputAntenna
    , videoInputComponent
    , videoInputHdmi
    , videoInputScart
    , videoInputSvideo
    , videoMarker
    , videoMarkerOutline
    , videoMinus
    , videoMinusOutline
    , videoOff
    , videoOffOutline
    , videoOutline
    , videoPlus
    , videoPlusOutline
    , videoStabilization
    , videoStandardDefinition
    , videoSwitch
    , videoSwitchOutline
    , videoVintage
    , videoWireless
    , videoWirelessOutline
    , viewAgenda
    , viewAgendaOutline
    , viewArray
    , viewArrayOutline
    , viewCarousel
    , viewCarouselOutline
    , viewColumn
    , viewColumnOutline
    , viewComfy
    , viewComfyOutline
    , viewCompact
    , viewCompactOutline
    , viewDashboard
    , viewDashboardEdit
    , viewDashboardEditOutline
    , viewDashboardOutline
    , viewDashboardVariant
    , viewDashboardVariantOutline
    , viewDay
    , viewDayOutline
    , viewGallery
    , viewGalleryOutline
    , viewGrid
    , viewGridCompact
    , viewGridOutline
    , viewGridPlus
    , viewGridPlusOutline
    , viewHeadline
    , viewList
    , viewListOutline
    , viewModule
    , viewModuleOutline
    , viewParallel
    , viewParallelOutline
    , viewQuilt
    , viewQuiltOutline
    , viewSequential
    , viewSequentialOutline
    , viewSplitHorizontal
    , viewSplitVertical
    , viewStream
    , viewStreamOutline
    , viewWeek
    , viewWeekOutline
    , violin
    , virtualReality
    , virus
    , virusOff
    , virusOffOutline
    , virusOutline
    , voicemail
    , volcano
    , volcanoOutline
    , volleyball
    , volumeEqual
    , volumeHigh
    , volumeLow
    , volumeMedium
    , volumeMinus
    , volumeMute
    , volumeOff
    , volumePlus
    , volumeSource
    , volumeVariantOff
    , volumeVibrate
    , vote
    , voteOutline
    , vpn
    )

import Material.Icons exposing (IconShape, iconShape)


{-| The [`vacuum`](https://pictogrammers.com/library/mdi/icon/vacuum/) icon.
-}
vacuum : IconShape
vacuum =
    iconShape "M23 20V22H16L16 20H18.46L12 4.61C11.81 4.14 11.5 3.76 11.06 3.46S10.14 3 9.61 3C8.9 3 8.28 3.27 7.76 3.79S7 4.92 7 5.64L7 9H8C10.21 9 12 10.79 12 13V22H8C8.61 21.16 9 20.13 9 19C9 16.24 6.76 14 4 14C3.29 14 2.61 14.15 2 14.42V9H5V5.64C5 4.8 5.23 4 5.63 3.32C6.04 2.62 6.59 2.06 7.3 1.63C8 1.21 8.77 1 9.61 1C10.55 1 11.4 1.26 12.16 1.77S13.5 2.97 13.87 3.81L20.66 20H23M7 19C7 20.66 5.66 22 4 22S1 20.66 1 19 2.34 16 4 16 7 17.34 7 19M5 19C5 18.45 4.55 18 4 18S3 18.45 3 19 3.45 20 4 20 5 19.55 5 19Z"


{-| The [`vacuum-outline`](https://pictogrammers.com/library/mdi/icon/vacuum-outline/) icon.
-}
vacuumOutline : IconShape
vacuumOutline =
    iconShape "M20.66 20L13.87 3.81C13.5 2.97 12.93 2.29 12.16 1.77C11.4 1.26 10.55 1 9.61 1C8.77 1 8 1.21 7.3 1.63S6.04 2.62 5.63 3.32 5 4.8 5 5.64L5.03 9H2.03V14.45C2.65 14.17 3.31 14.03 4 14.03V11.03H9C9.57 11.03 10.04 11.23 10.43 11.62C10.82 12 11 12.47 11 13V20.03H8.91C8.76 20.75 8.44 21.41 7.97 22H13V13C13 12.28 12.8 11.62 12.45 11S11.61 9.91 11 9.56C10.42 9.2 9.75 9 9 9H7V5.64C7 4.92 7.25 4.31 7.76 3.79S8.89 3 9.61 3C10.14 3 10.63 3.16 11.06 3.46S11.81 4.14 12 4.61L18.46 20L16 20.03V22H23V20.03L20.66 20M4 18C4.55 18 5 18.45 5 19S4.55 20 4 20 3 19.55 3 19 3.45 18 4 18M4 16C2.34 16 1 17.34 1 19S2.34 22 4 22 7 20.66 7 19 5.66 16 4 16Z"


{-| The [`valve`](https://pictogrammers.com/library/mdi/icon/valve/) icon.
-}
valve : IconShape
valve =
    iconShape "M4 22H2V2H4M22 2H20V22H22M17.24 5.34L13.24 9.34A3 3 0 0 0 9.24 13.34L5.24 17.34L6.66 18.76L10.66 14.76A3 3 0 0 0 14.66 10.76L18.66 6.76Z"


{-| The [`valve-closed`](https://pictogrammers.com/library/mdi/icon/valve-closed/) icon.
-}
valveClosed : IconShape
valveClosed =
    iconShape "M22 2V22H20V13H14.82A3 3 0 0 1 9.18 13H4V22H2V2H4V11H9.18A3 3 0 0 1 14.82 11H20V2Z"


{-| The [`valve-open`](https://pictogrammers.com/library/mdi/icon/valve-open/) icon.
-}
valveOpen : IconShape
valveOpen =
    iconShape "M4 22H2V2H4M22 2H20V22H22M11 4V9.18A3 3 0 0 0 11 14.82V20H13V14.82A3 3 0 0 0 13 9.18V4Z"


{-| The [`van-passenger`](https://pictogrammers.com/library/mdi/icon/van-passenger/) icon.
-}
vanPassenger : IconShape
vanPassenger =
    iconShape "M3,7C1.89,7 1,7.89 1,9V17H3A3,3 0 0,0 6,20A3,3 0 0,0 9,17H15A3,3 0 0,0 18,20A3,3 0 0,0 21,17H23V13C23,11.89 22.11,11 21,11L18,7H3M3,8.5H7V11H3V8.5M9,8.5H13V11H9V8.5M15,8.5H17.5L19.46,11H15V8.5M6,15.5A1.5,1.5 0 0,1 7.5,17A1.5,1.5 0 0,1 6,18.5A1.5,1.5 0 0,1 4.5,17A1.5,1.5 0 0,1 6,15.5M18,15.5A1.5,1.5 0 0,1 19.5,17A1.5,1.5 0 0,1 18,18.5A1.5,1.5 0 0,1 16.5,17A1.5,1.5 0 0,1 18,15.5Z"


{-| The [`van-utility`](https://pictogrammers.com/library/mdi/icon/van-utility/) icon.
-}
vanUtility : IconShape
vanUtility =
    iconShape "M3,7C1.89,7 1,7.89 1,9V17H3A3,3 0 0,0 6,20A3,3 0 0,0 9,17H15A3,3 0 0,0 18,20A3,3 0 0,0 21,17H23V13C23,11.89 22.11,11 21,11L18,7H3M15,8.5H17.5L19.46,11H15V8.5M6,15.5A1.5,1.5 0 0,1 7.5,17A1.5,1.5 0 0,1 6,18.5A1.5,1.5 0 0,1 4.5,17A1.5,1.5 0 0,1 6,15.5M18,15.5A1.5,1.5 0 0,1 19.5,17A1.5,1.5 0 0,1 18,18.5A1.5,1.5 0 0,1 16.5,17A1.5,1.5 0 0,1 18,15.5Z"


{-| The [`vanish`](https://pictogrammers.com/library/mdi/icon/vanish/) icon.
-}
vanish : IconShape
vanish =
    iconShape "M16,13V11H21V13H16M14.83,7.76L17.66,4.93L19.07,6.34L16.24,9.17L14.83,7.76M11,16H13V21H11V16M11,3H13V8H11V3M4.93,17.66L7.76,14.83L9.17,16.24L6.34,19.07L4.93,17.66M4.93,6.34L6.34,4.93L9.17,7.76L7.76,9.17L4.93,6.34M8,13H3V11H8V13M19.07,17.66L17.66,19.07L14.83,16.24L16.24,14.83L19.07,17.66Z"


{-| The [`vanish-quarter`](https://pictogrammers.com/library/mdi/icon/vanish-quarter/) icon.
-}
vanishQuarter : IconShape
vanishQuarter =
    iconShape "M11 3H13V8H11V3M4.9 6.3L6.3 4.9L9.1 7.7L7.8 9.2L4.9 6.3M8 13H3V11H8V13"


{-| The [`vanity-light`](https://pictogrammers.com/library/mdi/icon/vanity-light/) icon.
-}
vanityLight : IconShape
vanityLight =
    iconShape "M22 20H16C16 18.34 17.34 13 19 13S22 18.34 22 20M12 13C10.34 13 9 18.34 9 20H15C15 18.34 13.66 13 12 13M5 13C3.34 13 2 18.34 2 20H8C8 18.34 6.66 13 5 13M14.82 6C14.26 4.44 12.53 3.64 11 4.2C10.14 4.5 9.5 5.17 9.18 6H2V8H4V12H6V8H9.18C9.5 8.85 10.15 9.5 11 9.82V12H13V9.82C13.85 9.5 14.5 8.85 14.82 8H18V12H20V8H22V6H14.82Z"


{-| The [`variable`](https://pictogrammers.com/library/mdi/icon/variable/) icon.
-}
variable : IconShape
variable =
    iconShape "M20.41,3C21.8,5.71 22.35,8.84 22,12C21.8,15.16 20.7,18.29 18.83,21L17.3,20C18.91,17.57 19.85,14.8 20,12C20.34,9.2 19.89,6.43 18.7,4L20.41,3M5.17,3L6.7,4C5.09,6.43 4.15,9.2 4,12C3.66,14.8 4.12,17.57 5.3,20L3.61,21C2.21,18.29 1.65,15.17 2,12C2.2,8.84 3.3,5.71 5.17,3M12.08,10.68L14.4,7.45H16.93L13.15,12.45L15.35,17.37H13.09L11.71,14L9.28,17.33H6.76L10.66,12.21L8.53,7.45H10.8L12.08,10.68Z"


{-| The [`variable-box`](https://pictogrammers.com/library/mdi/icon/variable-box/) icon.
-}
variableBox : IconShape
variableBox =
    iconShape "M19 3H5C3.9 3 3 3.9 3 5V19C3 20.1 3.9 21 5 21H19C20.1 21 21 20.1 21 19V5C21 3.9 20.1 3 19 3M7.4 18C5.9 16.5 5 14.3 5 12S5.9 7.5 7.4 6L9 6.7C7.7 7.9 7 9.9 7 12S7.7 16.1 9 17.3L7.4 18M12.7 15L11.9 13L10.5 15H9L11.3 11.9L10 9H11.3L12.1 11L13.5 9H15L12.8 12L14.1 15H12.7M16.6 18L15 17.3C16.3 16 17 14.1 17 12S16.3 7.9 15 6.7L16.6 6C18.1 7.5 19 9.7 19 12S18.1 16.5 16.6 18Z"


{-| The [`vector-arrange-above`](https://pictogrammers.com/library/mdi/icon/vector-arrange-above/) icon.
-}
vectorArrangeAbove : IconShape
vectorArrangeAbove =
    iconShape "M3,1C1.89,1 1,1.89 1,3V14C1,15.11 1.89,16 3,16C6.67,16 10.33,16 14,16C15.11,16 16,15.11 16,14C16,10.33 16,6.67 16,3C16,1.89 15.11,1 14,1H3M3,3H14V14H3V3M18,7V9H20V20H9V18H7V20C7,21.11 7.89,22 9,22H20C21.11,22 22,21.11 22,20V9C22,7.89 21.11,7 20,7H18Z"


{-| The [`vector-arrange-below`](https://pictogrammers.com/library/mdi/icon/vector-arrange-below/) icon.
-}
vectorArrangeBelow : IconShape
vectorArrangeBelow =
    iconShape "M20,22C21.11,22 22,21.11 22,20V9C22,7.89 21.11,7 20,7C16.33,7 12.67,7 9,7C7.89,7 7,7.89 7,9C7,12.67 7,16.33 7,20C7,21.11 7.89,22 9,22H20M20,20H9V9H20V20M5,16V14H3V3H14V5H16V3C16,1.89 15.11,1 14,1H3C1.89,1 1,1.89 1,3V14C1,15.11 1.89,16 3,16H5Z"


{-| The [`vector-bezier`](https://pictogrammers.com/library/mdi/icon/vector-bezier/) icon.
-}
vectorBezier : IconShape
vectorBezier =
    iconShape "M7.5,4A1.5,1.5 0 0,0 6,5.5A1.5,1.5 0 0,0 7.5,7C8.13,7 8.7,6.6 8.91,6H13C13.67,5.33 14.33,5 15,5H8.91C8.7,4.4 8.13,4 7.5,4M19,5C8,5 14,17 5,17V19C16,19 10,7 19,7V5M16.5,17C15.87,17 15.3,17.4 15.09,18H11C10.33,18.67 9.67,19 9,19H15.09C15.3,19.6 15.87,20 16.5,20A1.5,1.5 0 0,0 18,18.5A1.5,1.5 0 0,0 16.5,17Z"


{-| The [`vector-circle`](https://pictogrammers.com/library/mdi/icon/vector-circle/) icon.
-}
vectorCircle : IconShape
vectorCircle =
    iconShape "M9,2V4.06C6.72,4.92 4.92,6.72 4.05,9H2V15H4.06C4.92,17.28 6.72,19.09 9,19.95V22H15V19.94C17.28,19.08 19.09,17.28 19.95,15H22V9H19.94C19.08,6.72 17.28,4.92 15,4.05V2M11,4H13V6H11M9,6.25V8H15V6.25C16.18,6.86 17.14,7.82 17.75,9H16V15H17.75C17.14,16.18 16.18,17.14 15,17.75V16H9V17.75C7.82,17.14 6.86,16.18 6.25,15H8V9H6.25C6.86,7.82 7.82,6.86 9,6.25M4,11H6V13H4M18,11H20V13H18M11,18H13V20H11"


{-| The [`vector-circle-variant`](https://pictogrammers.com/library/mdi/icon/vector-circle-variant/) icon.
-}
vectorCircleVariant : IconShape
vectorCircleVariant =
    iconShape "M22,9H19.97C18.7,5.41 15.31,3 11.5,3A9,9 0 0,0 2.5,12C2.5,17 6.53,21 11.5,21C15.31,21 18.7,18.6 20,15H22M20,11V13H18V11M17.82,15C16.66,17.44 14.2,19 11.5,19C7.64,19 4.5,15.87 4.5,12C4.5,8.14 7.64,5 11.5,5C14.2,5 16.66,6.57 17.81,9H16V15"


{-| The [`vector-combine`](https://pictogrammers.com/library/mdi/icon/vector-combine/) icon.
-}
vectorCombine : IconShape
vectorCombine =
    iconShape "M3,1C1.89,1 1,1.89 1,3V14C1,15.11 1.89,16 3,16C4.33,16 7,16 7,16C7,16 7,18.67 7,20C7,21.11 7.89,22 9,22H20C21.11,22 22,21.11 22,20V9C22,7.89 21.11,7 20,7C18.67,7 16,7 16,7C16,7 16,4.33 16,3C16,1.89 15.11,1 14,1H3M3,3H14C14,4.33 14,7 14,7H9C7.89,7 7,7.89 7,9V14C7,14 4.33,14 3,14V3M9,9H14V14H9V9M16,9C16,9 18.67,9 20,9V20H9C9,18.67 9,16 9,16H14C15.11,16 16,15.11 16,14V9Z"


{-| The [`vector-curve`](https://pictogrammers.com/library/mdi/icon/vector-curve/) icon.
-}
vectorCurve : IconShape
vectorCurve =
    iconShape "M18.5,2A1.5,1.5 0 0,1 20,3.5A1.5,1.5 0 0,1 18.5,5C18.27,5 18.05,4.95 17.85,4.85L14.16,8.55L14.5,9C16.69,7.74 19.26,7 22,7L23,7.03V9.04L22,9C19.42,9 17,9.75 15,11.04A3.96,3.96 0 0,1 11.04,15C9.75,17 9,19.42 9,22L9.04,23H7.03L7,22C7,19.26 7.74,16.69 9,14.5L8.55,14.16L4.85,17.85C4.95,18.05 5,18.27 5,18.5A1.5,1.5 0 0,1 3.5,20A1.5,1.5 0 0,1 2,18.5A1.5,1.5 0 0,1 3.5,17C3.73,17 3.95,17.05 4.15,17.15L7.84,13.45C7.31,12.78 7,11.92 7,11A4,4 0 0,1 11,7C11.92,7 12.78,7.31 13.45,7.84L17.15,4.15C17.05,3.95 17,3.73 17,3.5A1.5,1.5 0 0,1 18.5,2M11,9A2,2 0 0,0 9,11A2,2 0 0,0 11,13A2,2 0 0,0 13,11A2,2 0 0,0 11,9Z"


{-| The [`vector-difference`](https://pictogrammers.com/library/mdi/icon/vector-difference/) icon.
-}
vectorDifference : IconShape
vectorDifference =
    iconShape "M3,1C1.89,1 1,1.89 1,3V14C1,15.11 1.89,16 3,16H5V14H3V3H14V5H16V3C16,1.89 15.11,1 14,1H3M9,7C7.89,7 7,7.89 7,9V11H9V9H11V7H9M13,7V9H14V10H16V7H13M18,7V9H20V20H9V18H7V20C7,21.11 7.89,22 9,22H20C21.11,22 22,21.11 22,20V9C22,7.89 21.11,7 20,7H18M14,12V14H12V16H14C15.11,16 16,15.11 16,14V12H14M7,13V16H10V14H9V13H7Z"


{-| The [`vector-difference-ab`](https://pictogrammers.com/library/mdi/icon/vector-difference-ab/) icon.
-}
vectorDifferenceAb : IconShape
vectorDifferenceAb =
    iconShape "M3,1C1.89,1 1,1.89 1,3V5H3V3H5V1H3M7,1V3H10V1H7M12,1V3H14V5H16V3C16,1.89 15.11,1 14,1H12M1,7V10H3V7H1M14,7C14,7 14,11.67 14,14C11.67,14 7,14 7,14C7,14 7,18 7,20C7,21.11 7.89,22 9,22H20C21.11,22 22,21.11 22,20V9C22,7.89 21.11,7 20,7C18,7 14,7 14,7M16,9H20V20H9V16H14C15.11,16 16,15.11 16,14V9M1,12V14C1,15.11 1.89,16 3,16H5V14H3V12H1Z"


{-| The [`vector-difference-ba`](https://pictogrammers.com/library/mdi/icon/vector-difference-ba/) icon.
-}
vectorDifferenceBa : IconShape
vectorDifferenceBa =
    iconShape "M20,22C21.11,22 22,21.11 22,20V18H20V20H18V22H20M16,22V20H13V22H16M11,22V20H9V18H7V20C7,21.11 7.89,22 9,22H11M22,16V13H20V16H22M9,16C9,16 9,11.33 9,9C11.33,9 16,9 16,9C16,9 16,5 16,3C16,1.89 15.11,1 14,1H3C1.89,1 1,1.89 1,3V14C1,15.11 1.89,16 3,16C5,16 9,16 9,16M7,14H3V3H14V7H9C7.89,7 7,7.89 7,9V14M22,11V9C22,7.89 21.11,7 20,7H18V9H20V11H22Z"


{-| The [`vector-ellipse`](https://pictogrammers.com/library/mdi/icon/vector-ellipse/) icon.
-}
vectorEllipse : IconShape
vectorEllipse =
    iconShape "M23,9V15H20.35C19.38,17.12 17.43,18.78 15,19.54V22H9V19.54C5.5,18.45 3,15.5 3,12C3,7.58 7.03,4 12,4C15.78,4 19,6.07 20.35,9H23M17,15V9H18.06C16.85,7.21 14.59,6 12,6C8.13,6 5,8.69 5,12C5,14.39 6.64,16.46 9,17.42V16H15V17.42C16.29,16.9 17.35,16.05 18.06,15H17M19,13H21V11H19V13M11,20H13V18H11V20Z"


{-| The [`vector-intersection`](https://pictogrammers.com/library/mdi/icon/vector-intersection/) icon.
-}
vectorIntersection : IconShape
vectorIntersection =
    iconShape "M3.14,1A2.14,2.14 0 0,0 1,3.14V5H3V3H5V1H3.14M7,1V3H10V1H7M12,1V3H14V5H16V3.14C16,1.96 15.04,1 13.86,1H12M1,7V10H3V7H1M9,7C7.89,7 7,7.89 7,9C7,11.33 7,16 7,16C7,16 11.57,16 13.86,16A2.14,2.14 0 0,0 16,13.86C16,11.57 16,7 16,7C16,7 11.33,7 9,7M18,7V9H20V11H22V9C22,7.89 21.11,7 20,7H18M9,9H14V14H9V9M1,12V13.86C1,15.04 1.96,16 3.14,16H5V14H3V12H1M20,13V16H22V13H20M7,18V20C7,21.11 7.89,22 9,22H11V20H9V18H7M20,18V20H18V22H20C21.11,22 22,21.11 22,20V18H20M13,20V22H16V20H13Z"


{-| The [`vector-line`](https://pictogrammers.com/library/mdi/icon/vector-line/) icon.
-}
vectorLine : IconShape
vectorLine =
    iconShape "M15,3V7.59L7.59,15H3V21H9V16.42L16.42,9H21V3M17,5H19V7H17M5,17H7V19H5"


{-| The [`vector-link`](https://pictogrammers.com/library/mdi/icon/vector-link/) icon.
-}
vectorLink : IconShape
vectorLink =
    iconShape "M3 1C1.89 1 1 1.89 1 3V14C1 15.11 1.89 16 3 16H14C15.11 16 16 15.11 16 14V11H14V14H3V3H14V5H16V3C16 1.89 15.11 1 14 1M9 7C7.89 7 7 7.89 7 9V12H9V9H20V20H9V18H7V20C7 21.11 7.89 22 9 22H20C21.11 22 22 21.11 22 20V9C22 7.89 21.11 7 20 7H9"


{-| The [`vector-point`](https://pictogrammers.com/library/mdi/icon/vector-point/) icon.
-}
vectorPoint : IconShape
vectorPoint =
    iconShape "M9 9V15H15V9H9M11 11H13V13H11V11Z"


{-| The [`vector-point-edit`](https://pictogrammers.com/library/mdi/icon/vector-point-edit/) icon.
-}
vectorPointEdit : IconShape
vectorPointEdit =
    iconShape "M9 9V15H15V9H9M11 11H13V13H11V11M21.2 13C21.1 13 20.9 13.1 20.8 13.2L19.8 14.2L21.9 16.3L22.9 15.3C23.1 15.1 23.1 14.7 22.9 14.5L21.6 13.2C21.4 13.1 21.3 13 21.2 13M19.1 14.8L13 20.9V23H15.1L21.2 16.8L19.1 14.8Z"


{-| The [`vector-point-minus`](https://pictogrammers.com/library/mdi/icon/vector-point-minus/) icon.
-}
vectorPointMinus : IconShape
vectorPointMinus =
    iconShape "M9 9V15H15V9H9M11 11H13V13H11V11M15 18V20H23V18H15Z"


{-| The [`vector-point-plus`](https://pictogrammers.com/library/mdi/icon/vector-point-plus/) icon.
-}
vectorPointPlus : IconShape
vectorPointPlus =
    iconShape "M9 9V15H15V9H9M11 11H13V13H11V11M18 15V18H15V20H18V23H20V20H23V18H20V15H18Z"


{-| The [`vector-point-select`](https://pictogrammers.com/library/mdi/icon/vector-point-select/) icon.
-}
vectorPointSelect : IconShape
vectorPointSelect =
    iconShape "M12,20L7,22L12,11L17,22L12,20M8,2H16V5H22V7H16V10H8V7H2V5H8V2M10,4V8H14V4H10Z"


{-| The [`vector-polygon`](https://pictogrammers.com/library/mdi/icon/vector-polygon/) icon.
-}
vectorPolygon : IconShape
vectorPolygon =
    iconShape "M2,2V8H4.28L5.57,16H4V22H10V20.06L15,20.05V22H21V16H19.17L20,9H22V3H16V6.53L14.8,8H9.59L8,5.82V2M4,4H6V6H4M18,5H20V7H18M6.31,8H7.11L9,10.59V14H15V10.91L16.57,9H18L17.16,16H15V18.06H10V16H7.6M11,10H13V12H11M6,18H8V20H6M17,18H19V20H17"


{-| The [`vector-polygon-variant`](https://pictogrammers.com/library/mdi/icon/vector-polygon-variant/) icon.
-}
vectorPolygonVariant : IconShape
vectorPolygonVariant =
    iconShape "M22 8V2H16V5.8L14.4 8H9.6L8 5.8V2H2V8H4V16H2V22H8V20H16V22H22V16H20V8H22M11 10H13V12H11V10M4 4H6V6H4V4M6 20H4V18H6V20M16 18H8V16H6V8H7.1L9 10.6V14H15V10.6L16.9 8H18V16H16V18M20 20H18V18H20V20M18 6V4H20V6H18Z"


{-| The [`vector-polyline`](https://pictogrammers.com/library/mdi/icon/vector-polyline/) icon.
-}
vectorPolyline : IconShape
vectorPolyline =
    iconShape "M2 3V9H4.95L6.95 15H6V21H12V16.41L17.41 11H22V5H16V9.57L10.59 15H9.06L7.06 9H8V3M4 5H6V7H4M18 7H20V9H18M8 17H10V19H8Z"


{-| The [`vector-polyline-edit`](https://pictogrammers.com/library/mdi/icon/vector-polyline-edit/) icon.
-}
vectorPolylineEdit : IconShape
vectorPolylineEdit =
    iconShape "M2 3V9H4.95L6.95 15H6V21H12V16.41L17.41 11H22V5H16V9.57L10.59 15H9.06L7.06 9H8V3H2M4 5H6V7H4V5M18 7H20V9H18V7M21.15 13C21 13 20.87 13.05 20.76 13.16L19.75 14.18L21.82 16.25L22.84 15.24C23.05 15.03 23.05 14.67 22.84 14.46L21.54 13.16C21.43 13.05 21.29 13 21.15 13M19.15 14.76L13 20.91V23H15.09L21.23 16.84L19.15 14.76M8 17H10V19H8V17Z"


{-| The [`vector-polyline-minus`](https://pictogrammers.com/library/mdi/icon/vector-polyline-minus/) icon.
-}
vectorPolylineMinus : IconShape
vectorPolylineMinus =
    iconShape "M2 3V9H4.95L6.95 15H6V21H12V16.41L17.41 11H22V5H16V9.57L10.59 15H9.06L7.06 9H8V3H2M4 5H6V7H4V5M18 7H20V9H18V7M8 17H10V19H8V17M15 18V20H23V18H15Z"


{-| The [`vector-polyline-plus`](https://pictogrammers.com/library/mdi/icon/vector-polyline-plus/) icon.
-}
vectorPolylinePlus : IconShape
vectorPolylinePlus =
    iconShape "M2 3V9H4.95L6.95 15H6V21H12V16.41L17.41 11H22V5H16V9.57L10.59 15H9.06L7.06 9H8V3H2M4 5H6V7H4V5M18 7H20V9H18V7M18 15V18H15V20H18V23H20V20H23V18H20V15H18M8 17H10V19H8V17Z"


{-| The [`vector-polyline-remove`](https://pictogrammers.com/library/mdi/icon/vector-polyline-remove/) icon.
-}
vectorPolylineRemove : IconShape
vectorPolylineRemove =
    iconShape "M21.1 15.5L19 17.6L16.9 15.5L15.5 16.9L17.6 19L15.5 21.1L16.9 22.5L19 20.4L21.1 22.5L22.5 21.1L20.4 19L22.5 16.9L21.1 15.5M16 5V9.6L10.6 15H9.1L7.1 9H8V3H2V9H5L7 15H6V21H12V16.4L17.4 11H22V5H16M6 7H4V5H6V7M10 19H8V17H10V19M20 9H18V7H20V9"


{-| The [`vector-radius`](https://pictogrammers.com/library/mdi/icon/vector-radius/) icon.
-}
vectorRadius : IconShape
vectorRadius =
    iconShape "M2,4H4V2H10V4A10,10 0 0,1 20,14H22V20H20V22H18V20H16V14H18A8,8 0 0,0 10,6V8H4V6H2V4M18,16V18H20V16H18M6,4V6H8V4H6Z"


{-| The [`vector-rectangle`](https://pictogrammers.com/library/mdi/icon/vector-rectangle/) icon.
-}
vectorRectangle : IconShape
vectorRectangle =
    iconShape "M2,4H8V6H16V4H22V10H20V14H22V20H16V18H8V20H2V14H4V10H2V4M16,10V8H8V10H6V14H8V16H16V14H18V10H16M4,6V8H6V6H4M18,6V8H20V6H18M4,16V18H6V16H4M18,16V18H20V16H18Z"


{-| The [`vector-selection`](https://pictogrammers.com/library/mdi/icon/vector-selection/) icon.
-}
vectorSelection : IconShape
vectorSelection =
    iconShape "M3,1H5V3H3V5H1V3A2,2 0 0,1 3,1M14,1A2,2 0 0,1 16,3V5H14V3H12V1H14M20,7A2,2 0 0,1 22,9V11H20V9H18V7H20M22,20A2,2 0 0,1 20,22H18V20H20V18H22V20M20,13H22V16H20V13M13,9V7H16V10H14V9H13M13,22V20H16V22H13M9,22A2,2 0 0,1 7,20V18H9V20H11V22H9M7,16V13H9V14H10V16H7M7,3V1H10V3H7M3,16A2,2 0 0,1 1,14V12H3V14H5V16H3M1,7H3V10H1V7M9,7H11V9H9V11H7V9A2,2 0 0,1 9,7M16,14A2,2 0 0,1 14,16H12V14H14V12H16V14Z"


{-| The [`vector-square`](https://pictogrammers.com/library/mdi/icon/vector-square/) icon.
-}
vectorSquare : IconShape
vectorSquare =
    iconShape "M2,2H8V4H16V2H22V8H20V16H22V22H16V20H8V22H2V16H4V8H2V2M16,8V6H8V8H6V16H8V18H16V16H18V8H16M4,4V6H6V4H4M18,4V6H20V4H18M4,18V20H6V18H4M18,18V20H20V18H18Z"


{-| The [`vector-square-close`](https://pictogrammers.com/library/mdi/icon/vector-square-close/) icon.
-}
vectorSquareClose : IconShape
vectorSquareClose =
    iconShape "M4 4H6V6H4V4M6 20H4V18H6V20M18 8V16H16V18H8V16H6V8H8V2H2V8H4V16H2V22H8V20H16V22H22V16H20V8H22V2H16V8H18M20 20H18V18H20V20M18 6V4H20V6H18M14 6H10V4H14V6Z"


{-| The [`vector-square-edit`](https://pictogrammers.com/library/mdi/icon/vector-square-edit/) icon.
-}
vectorSquareEdit : IconShape
vectorSquareEdit =
    iconShape "M22.7 14.4L21.7 15.4L19.6 13.3L20.6 12.3C20.8 12.1 21.2 12.1 21.4 12.3L22.7 13.6C22.9 13.8 22.9 14.1 22.7 14.4M13 19.9L19.1 13.8L21.2 15.9L15.1 22H13V19.9M11 19.9V19.1L11.6 18.5L12.1 18H8V16H6V8H8V6H16V8H18V12.1L19.1 11L19.3 10.8C19.5 10.6 19.8 10.4 20.1 10.3V8H22.1V2H16.1V4H8V2H2V8H4V16H2V22H8V20L11 19.9M18 4H20V6H18V4M4 4H6V6H4V4M6 20H4V18H6V20Z"


{-| The [`vector-square-minus`](https://pictogrammers.com/library/mdi/icon/vector-square-minus/) icon.
-}
vectorSquareMinus : IconShape
vectorSquareMinus =
    iconShape "M13 19C13 18.7 13 18.3 13.1 18H8V16H6V8H8V6H16V8H18V13.1C18.3 13 18.7 13 19 13C19.3 13 19.7 13 20 13.1V8H22V2H16V4H8V2H2V8H4V16H2V22H8V20H13.1C13 19.7 13 19.3 13 19M18 4H20V6H18V4M4 4H6V6H4V4M6 20H4V18H6V20M23 18V20H15V18H23Z"


{-| The [`vector-square-open`](https://pictogrammers.com/library/mdi/icon/vector-square-open/) icon.
-}
vectorSquareOpen : IconShape
vectorSquareOpen =
    iconShape "M4 4H6V6H4V4M6 20H4V18H6V20M18 8V16H16V18H8V16H6V8H8V2H2V8H4V16H2V22H8V20H16V22H22V16H20V8H22V2H16V8H18M20 20H18V18H20V20M18 6V4H20V6H18Z"


{-| The [`vector-square-plus`](https://pictogrammers.com/library/mdi/icon/vector-square-plus/) icon.
-}
vectorSquarePlus : IconShape
vectorSquarePlus =
    iconShape "M13 19C13 18.7 13 18.3 13.1 18H8V16H6V8H8V6H16V8H18V13.1C18.3 13 18.7 13 19 13C19.3 13 19.7 13 20 13.1V8H22V2H16V4H8V2H2V8H4V16H2V22H8V20H13.1C13 19.7 13 19.3 13 19M18 4H20V6H18V4M4 4H6V6H4V4M6 20H4V18H6V20M20 15V18H23V20H20V23H18V20H15V18H18V15H20Z"


{-| The [`vector-square-remove`](https://pictogrammers.com/library/mdi/icon/vector-square-remove/) icon.
-}
vectorSquareRemove : IconShape
vectorSquareRemove =
    iconShape "M13 19C13 18.7 13 18.3 13.1 18H8V16H6V8H8V6H16V8H18V13.1C18.3 13 18.7 13 19 13C19.3 13 19.7 13 20 13.1V8H22V2H16V4H8V2H2V8H4V16H2V22H8V20H13.1C13 19.7 13 19.3 13 19M18 4H20V6H18V4M4 4H6V6H4V4M6 20H4V18H6V20M22.5 16.9L20.4 19L22.5 21.1L21.1 22.5L19 20.4L16.9 22.5L15.5 21.1L17.6 19L15.5 16.9L16.9 15.5L19 17.6L21.1 15.5L22.5 16.9Z"


{-| The [`vector-triangle`](https://pictogrammers.com/library/mdi/icon/vector-triangle/) icon.
-}
vectorTriangle : IconShape
vectorTriangle =
    iconShape "M9,3V9H9.73L5.79,16H2V22H8V20H16V22H22V16H18.21L14.27,9H15V3M11,5H13V7H11M12,9.04L16,16.15V18H8V16.15M4,18H6V20H4M18,18H20V20H18"


{-| The [`vector-union`](https://pictogrammers.com/library/mdi/icon/vector-union/) icon.
-}
vectorUnion : IconShape
vectorUnion =
    iconShape "M3,1C1.89,1 1,1.89 1,3V14C1,15.11 1.89,16 3,16H7V20C7,21.11 7.89,22 9,22H20C21.11,22 22,21.11 22,20V9C22,7.89 21.11,7 20,7H16V3C16,1.89 15.11,1 14,1H3M3,3H14V9H20V20H9V14H3V3Z"


{-| The [`vhs`](https://pictogrammers.com/library/mdi/icon/vhs/) icon.
-}
vhs : IconShape
vhs =
    iconShape "M4,6A2,2 0 0,0 2,8V16A2,2 0 0,0 4,18H20A2,2 0 0,0 22,16V8A2,2 0 0,0 20,6H4M4.54,10H7V14H4.54C4.19,13.39 4,12.7 4,12C4,11.3 4.19,10.61 4.54,10M9,10H15V14H9V10M17,10H19.46C19.81,10.61 20,11.3 20,12C20,12.7 19.81,13.39 19.46,14H17V10Z"


{-| The [`vibrate`](https://pictogrammers.com/library/mdi/icon/vibrate/) icon.
-}
vibrate : IconShape
vibrate =
    iconShape "M16,19H8V5H16M16.5,3H7.5A1.5,1.5 0 0,0 6,4.5V19.5A1.5,1.5 0 0,0 7.5,21H16.5A1.5,1.5 0 0,0 18,19.5V4.5A1.5,1.5 0 0,0 16.5,3M19,17H21V7H19M22,9V15H24V9M3,17H5V7H3M0,15H2V9H0V15Z"


{-| The [`vibrate-off`](https://pictogrammers.com/library/mdi/icon/vibrate-off/) icon.
-}
vibrateOff : IconShape
vibrateOff =
    iconShape "M8.2,5L6.55,3.35C6.81,3.12 7.15,3 7.5,3H16.5A1.5,1.5 0 0,1 18,4.5V14.8L16,12.8V5H8.2M0,15H2V9H0V15M21,17V7H19V15.8L20.2,17H21M3,17H5V7H3V17M18,17.35L22.11,21.46L20.84,22.73L18,19.85C17.83,20.54 17.21,21 16.5,21H7.5A1.5,1.5 0 0,1 6,19.5V7.89L1.11,3L2.39,1.73L6.09,5.44L8,7.34L16,15.34L18,17.34V17.35M16,17.89L8,9.89V19H16V17.89M22,9V15H24V9H22Z"


{-| The [`video`](https://pictogrammers.com/library/mdi/icon/video/) icon.
-}
video : IconShape
video =
    iconShape "M17,10.5V7A1,1 0 0,0 16,6H4A1,1 0 0,0 3,7V17A1,1 0 0,0 4,18H16A1,1 0 0,0 17,17V13.5L21,17.5V6.5L17,10.5Z"


{-| The [`video-2d`](https://pictogrammers.com/library/mdi/icon/video-2d/) icon.
-}
video2d : IconShape
video2d =
    iconShape "M13 7H16C17.66 7 19 8.34 19 10V14C19 15.66 17.66 17 16 17H13V7M16 15C16.55 15 17 14.55 17 14V10C17 9.45 16.55 9 16 9H15V15H16M5 7H9C10.11 7 11 7.9 11 9V11C11 12.11 10.11 13 9 13H7V15H11V17H5V13C5 11.9 5.9 11 7 11H9V9H5V7Z"


{-| The [`video-3d`](https://pictogrammers.com/library/mdi/icon/video-3d/) icon.
-}
video3d : IconShape
video3d =
    iconShape "M5,7H9A2,2 0 0,1 11,9V15A2,2 0 0,1 9,17H5V15H9V13H6V11H9V9H5V7M13,7H16A3,3 0 0,1 19,10V14A3,3 0 0,1 16,17H13V7M16,15A1,1 0 0,0 17,14V10A1,1 0 0,0 16,9H15V15H16Z"


{-| The [`video-3d-off`](https://pictogrammers.com/library/mdi/icon/video-3d-off/) icon.
-}
video3dOff : IconShape
video3dOff =
    iconShape "M2.61 2L21.35 20.74L19.94 22.15L14.8 17H13V15.2L11 13.23V15.21C10.93 15.68 10.74 16.08 10.41 16.41C10.08 16.73 9.68 16.93 9.21 17H5V15H9V13H6V11H8.8L6.79 9H5V7.23L1.2 3.41L2.61 2M16 7H16.22C17 7.07 17.63 7.36 18.14 7.88C18.65 8.39 18.93 9 19 9.76V14.24C18.95 14.63 18.87 15 18.72 15.3L17 13.6V9.85C16.95 9.63 16.84 9.44 16.69 9.28C16.53 9.13 16.34 9.03 16.12 9H15V11.59L13 9.57V7H16Z"


{-| The [`video-3d-variant`](https://pictogrammers.com/library/mdi/icon/video-3d-variant/) icon.
-}
video3dVariant : IconShape
video3dVariant =
    iconShape "M14,10V14A0.5,0.5 0 0,1 13.5,14.5H12.5V9.5H13.5A0.5,0.5 0 0,1 14,10M21,6.5V17.5L17,13.5V17A1,1 0 0,1 16,18H4A1,1 0 0,1 3,17V7A1,1 0 0,1 4,6H16A1,1 0 0,1 17,7V10.5M9.5,9.5A1.5,1.5 0 0,0 8,8H4.5V9.5H8V11.25H5.5V12.75H8V14.5H4.5V16H8A1.5,1.5 0 0,0 9.5,14.5M15.5,9.5A1.5,1.5 0 0,0 14,8H11V16H14A1.5,1.5 0 0,0 15.5,14.5"


{-| The [`video-4k-box`](https://pictogrammers.com/library/mdi/icon/video-4k-box/) icon.
-}
video4kBox : IconShape
video4kBox =
    iconShape "M19,3H5A2,2 0 0,0 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19V5A2,2 0 0,0 19,3M12,13.5H11V15H9.5V13.5H6.5V9H8V12H9.5V9H11V12H12V13.5M18,15H16.2L14.4,12.8V15H13V9H14.5V11.2L16.2,9H18L15.8,12L18,15Z"


{-| The [`video-account`](https://pictogrammers.com/library/mdi/icon/video-account/) icon.
-}
videoAccount : IconShape
videoAccount =
    iconShape "M17,10.5L21,6.5V17.5L17,13.5V17A1,1 0 0,1 16,18H4A1,1 0 0,1 3,17V7A1,1 0 0,1 4,6H16A1,1 0 0,1 17,7V10.5M14,16V15C14,13.67 11.33,13 10,13C8.67,13 6,13.67 6,15V16H14M10,8A2,2 0 0,0 8,10A2,2 0 0,0 10,12A2,2 0 0,0 12,10A2,2 0 0,0 10,8Z"


{-| The [`video-box`](https://pictogrammers.com/library/mdi/icon/video-box/) icon.
-}
videoBox : IconShape
videoBox =
    iconShape "M18,16L14,12.8V16H6V8H14V11.2L18,8M20,4H4A2,2 0 0,0 2,6V18A2,2 0 0,0 4,20H20A2,2 0 0,0 22,18V6C22,4.89 21.1,4 20,4Z"


{-| The [`video-box-off`](https://pictogrammers.com/library/mdi/icon/video-box-off/) icon.
-}
videoBoxOff : IconShape
videoBoxOff =
    iconShape "M6,8H6.73L14,15.27V16H6M2.27,1L1,2.27L3,4.28C2.41,4.62 2,5.26 2,6V18A2,2 0 0,0 4,20H18.73L20.73,22L22,20.73M20,4H7.82L11.82,8H14V10.18L14.57,10.75L18,8V14.18L22,18.17C22,18.11 22,18.06 22,18V6A2,2 0 0,0 20,4Z"


{-| The [`video-check`](https://pictogrammers.com/library/mdi/icon/video-check/) icon.
-}
videoCheck : IconShape
videoCheck =
    iconShape "M17 10.5V7C17 6.45 16.55 6 16 6H4C3.45 6 3 6.45 3 7V17C3 17.55 3.45 18 4 18H16C16.55 18 17 17.55 17 17V13.5L21 17.5V6.5L17 10.5M8.93 15L6 11.8L7.24 10.56L8.93 12.26L12.76 8.43L14 9.93L8.93 15Z"


{-| The [`video-check-outline`](https://pictogrammers.com/library/mdi/icon/video-check-outline/) icon.
-}
videoCheckOutline : IconShape
videoCheckOutline =
    iconShape "M15 8V16H5V8H15M16 6H4C3.45 6 3 6.45 3 7V17C3 17.55 3.45 18 4 18H16C16.55 18 17 17.55 17 17V13.5L21 17.5V6.5L17 10.5V7C17 6.45 16.55 6 16 6M6.35 12.08L7.5 10.95L9 12.5L12.5 9L13.65 10.37L9 15L6.35 12.08Z"


{-| The [`video-high-definition`](https://pictogrammers.com/library/mdi/icon/video-high-definition/) icon.
-}
videoHighDefinition : IconShape
videoHighDefinition =
    iconShape "M14 10V14C14 14.3 13.8 14.5 13.5 14.5H12.5V9.5H13.5C13.8 9.5 14 9.7 14 10M17 10.5V7C17 6.4 16.6 6 16 6H4C3.4 6 3 6.4 3 7V17C3 17.6 3.4 18 4 18H16C16.6 18 17 17.6 17 17V13.5L21 17.5V6.5L17 10.5M9.5 16H8V12.8H6V16H4.5V8H6V11.2H8V8H9.5V16M15.5 14.5C15.5 15.3 14.8 16 14 16H11V8H14C14.8 8 15.5 8.7 15.5 9.5V14.5Z"


{-| The [`video-image`](https://pictogrammers.com/library/mdi/icon/video-image/) icon.
-}
videoImage : IconShape
videoImage =
    iconShape "M17,10.5L21,6.5V17.5L17,13.5V17A1,1 0 0,1 16,18H4A1,1 0 0,1 3,17V7A1,1 0 0,1 4,6H16A1,1 0 0,1 17,7V10.5M10.91,9.55L9.2,11.82L10.5,13.55L9.77,14.09L7.73,11.36L5,15H15L10.91,9.55Z"


{-| The [`video-input-antenna`](https://pictogrammers.com/library/mdi/icon/video-input-antenna/) icon.
-}
videoInputAntenna : IconShape
videoInputAntenna =
    iconShape "M12,5A7,7 0 0,0 5,12H7A5,5 0 0,1 12,7A5,5 0 0,1 17,12H19A7,7 0 0,0 12,5M13,14.29C13.88,13.9 14.5,13.03 14.5,12A2.5,2.5 0 0,0 12,9.5A2.5,2.5 0 0,0 9.5,12C9.5,13 10.12,13.9 11,14.29V17.59L7.59,21L9,22.41L12,19.41L15,22.41L16.41,21L13,17.59V14.29M12,1A11,11 0 0,0 1,12H3A9,9 0 0,1 12,3A9,9 0 0,1 21,12H23A11,11 0 0,0 12,1Z"


{-| The [`video-input-component`](https://pictogrammers.com/library/mdi/icon/video-input-component/) icon.
-}
videoInputComponent : IconShape
videoInputComponent =
    iconShape "M5,2A1,1 0 0,0 4,1A1,1 0 0,0 3,2V6H1V12H7V6H5V2M9,16C9,17.3 9.84,18.4 11,18.82V23H13V18.82C14.16,18.41 15,17.31 15,16V14H9V16M1,16C1,17.3 1.84,18.4 3,18.82V23H5V18.82C6.16,18.4 7,17.3 7,16V14H1V16M21,6V2A1,1 0 0,0 20,1A1,1 0 0,0 19,2V6H17V12H23V6H21M13,2A1,1 0 0,0 12,1A1,1 0 0,0 11,2V6H9V12H15V6H13V2M17,16C17,17.3 17.84,18.4 19,18.82V23H21V18.82C22.16,18.41 23,17.31 23,16V14H17V16Z"


{-| The [`video-input-hdmi`](https://pictogrammers.com/library/mdi/icon/video-input-hdmi/) icon.
-}
videoInputHdmi : IconShape
videoInputHdmi =
    iconShape "M18,7V4A2,2 0 0,0 16,2H8A2,2 0 0,0 6,4V7H5V13L8,19V22H16V19L19,13V7H18M8,4H16V7H14V5H13V7H11V5H10V7H8V4Z"


{-| The [`video-input-scart`](https://pictogrammers.com/library/mdi/icon/video-input-scart/) icon.
-}
videoInputScart : IconShape
videoInputScart =
    iconShape "M20.6 2.2L17.3 2.4L13.8 4.4L13.3 3.5L2 10V17H3V19C3 20.1 3.9 21 5 21H15C16.1 21 17 20.1 17 19V17H18V10H17L16.8 9.6L20.3 7.6L22.1 4.8L20.6 2.2M15 17V19H5V17H15Z"


{-| The [`video-input-svideo`](https://pictogrammers.com/library/mdi/icon/video-input-svideo/) icon.
-}
videoInputSvideo : IconShape
videoInputSvideo =
    iconShape "M8,11.5A1.5,1.5 0 0,0 6.5,10A1.5,1.5 0 0,0 5,11.5A1.5,1.5 0 0,0 6.5,13A1.5,1.5 0 0,0 8,11.5M15,6.5A1.5,1.5 0 0,0 13.5,5H10.5A1.5,1.5 0 0,0 9,6.5A1.5,1.5 0 0,0 10.5,8H13.5A1.5,1.5 0 0,0 15,6.5M8.5,15A1.5,1.5 0 0,0 7,16.5A1.5,1.5 0 0,0 8.5,18A1.5,1.5 0 0,0 10,16.5A1.5,1.5 0 0,0 8.5,15M12,1A11,11 0 0,0 1,12A11,11 0 0,0 12,23A11,11 0 0,0 23,12A11,11 0 0,0 12,1M12,21C7.04,21 3,16.96 3,12C3,7.04 7.04,3 12,3C16.96,3 21,7.04 21,12C21,16.96 16.96,21 12,21M17.5,10A1.5,1.5 0 0,0 16,11.5A1.5,1.5 0 0,0 17.5,13A1.5,1.5 0 0,0 19,11.5A1.5,1.5 0 0,0 17.5,10M15.5,15A1.5,1.5 0 0,0 14,16.5A1.5,1.5 0 0,0 15.5,18A1.5,1.5 0 0,0 17,16.5A1.5,1.5 0 0,0 15.5,15Z"


{-| The [`video-marker`](https://pictogrammers.com/library/mdi/icon/video-marker/) icon.
-}
videoMarker : IconShape
videoMarker =
    iconShape "M5.5 16.8C4.8 16.8 4.3 16.2 4.3 15.6C4.3 14.9 4.9 14.4 5.5 14.4S6.7 15 6.7 15.6C6.8 16.2 6.2 16.8 5.5 16.8M5.5 12C3.6 12 2 13.6 2 15.5C2 18.1 5.5 22 5.5 22S9 18.1 9 15.5C9 13.6 7.4 12 5.5 12M4 6C3.4 6 3 6.4 3 7V10.6C3.8 10.2 4.6 10 5.5 10C8.5 10 11 12.5 11 15.5C11 16.4 10.8 17.2 10.5 18H16C16.6 18 17 17.6 17 17V13.5L21 17.5V6.5L17 10.5V7C17 6.4 16.6 6 16 6H4Z"


{-| The [`video-marker-outline`](https://pictogrammers.com/library/mdi/icon/video-marker-outline/) icon.
-}
videoMarkerOutline : IconShape
videoMarkerOutline =
    iconShape "M5.5 16.8C4.8 16.8 4.3 16.2 4.3 15.6C4.3 14.9 4.9 14.4 5.5 14.4S6.7 15 6.7 15.6C6.8 16.2 6.2 16.8 5.5 16.8M5.5 12C3.6 12 2 13.6 2 15.5C2 18.1 5.5 22 5.5 22S9 18.1 9 15.5C9 13.6 7.4 12 5.5 12M4 6C3.4 6 3 6.4 3 7V10.6C3.6 10.3 4.3 10.1 5 10V8H15V16H11C10.9 16.7 10.8 17.4 10.5 18H16C16.6 18 17 17.6 17 17V13.5L21 17.5V6.5L17 10.5V7C17 6.4 16.6 6 16 6H4Z"


{-| The [`video-minus`](https://pictogrammers.com/library/mdi/icon/video-minus/) icon.
-}
videoMinus : IconShape
videoMinus =
    iconShape "M17,10.5V7A1,1 0 0,0 16,6H4A1,1 0 0,0 3,7V17A1,1 0 0,0 4,18H16A1,1 0 0,0 17,17V13.5L21,17.5V6.5L17,10.5M14,13H6V11H14V13Z"


{-| The [`video-minus-outline`](https://pictogrammers.com/library/mdi/icon/video-minus-outline/) icon.
-}
videoMinusOutline : IconShape
videoMinusOutline =
    iconShape "M15 8V16H5V8H15M16 6H4C3.45 6 3 6.45 3 7V17C3 17.55 3.45 18 4 18H16C16.55 18 17 17.55 17 17V13.5L21 17.5V6.5L17 10.5V7C17 6.45 16.55 6 16 6M13 13H7V11H13V13Z"


{-| The [`video-off`](https://pictogrammers.com/library/mdi/icon/video-off/) icon.
-}
videoOff : IconShape
videoOff =
    iconShape "M3.27,2L2,3.27L4.73,6H4A1,1 0 0,0 3,7V17A1,1 0 0,0 4,18H16C16.2,18 16.39,17.92 16.54,17.82L19.73,21L21,19.73M21,6.5L17,10.5V7A1,1 0 0,0 16,6H9.82L21,17.18V6.5Z"


{-| The [`video-off-outline`](https://pictogrammers.com/library/mdi/icon/video-off-outline/) icon.
-}
videoOffOutline : IconShape
videoOffOutline =
    iconShape "M3.41,1.86L2,3.27L4.73,6H4A1,1 0 0,0 3,7V17A1,1 0 0,0 4,18H16C16.21,18 16.39,17.92 16.55,17.82L19.73,21L21.14,19.59L12.28,10.73L3.41,1.86M5,16V8H6.73L14.73,16H5M15,8V10.61L21,16.61V6.5L17,10.5V7A1,1 0 0,0 16,6H10.39L12.39,8H15Z"


{-| The [`video-outline`](https://pictogrammers.com/library/mdi/icon/video-outline/) icon.
-}
videoOutline : IconShape
videoOutline =
    iconShape "M15,8V16H5V8H15M16,6H4A1,1 0 0,0 3,7V17A1,1 0 0,0 4,18H16A1,1 0 0,0 17,17V13.5L21,17.5V6.5L17,10.5V7A1,1 0 0,0 16,6Z"


{-| The [`video-plus`](https://pictogrammers.com/library/mdi/icon/video-plus/) icon.
-}
videoPlus : IconShape
videoPlus =
    iconShape "M17,10.5V7A1,1 0 0,0 16,6H4A1,1 0 0,0 3,7V17A1,1 0 0,0 4,18H16A1,1 0 0,0 17,17V13.5L21,17.5V6.5L17,10.5M14,13H11V16H9V13H6V11H9V8H11V11H14V13Z"


{-| The [`video-plus-outline`](https://pictogrammers.com/library/mdi/icon/video-plus-outline/) icon.
-}
videoPlusOutline : IconShape
videoPlusOutline =
    iconShape "M15 8V16H5V8H15M16 6H4C3.45 6 3 6.45 3 7V17C3 17.55 3.45 18 4 18H16C16.55 18 17 17.55 17 17V13.5L21 17.5V6.5L17 10.5V7C17 6.45 16.55 6 16 6M13 13H11V15H9V13H7V11H9V9H11V11H13V13Z"


{-| The [`video-stabilization`](https://pictogrammers.com/library/mdi/icon/video-stabilization/) icon.
-}
videoStabilization : IconShape
videoStabilization =
    iconShape "M4,4H20A2,2 0 0,1 22,6V18A2,2 0 0,1 20,20H4A2,2 0 0,1 2,18V6A2,2 0 0,1 4,4M4,6V18H20V6H4M7.24,6.58L18.83,9.69L16.76,17.42L5.17,14.31L7.24,6.58Z"


{-| The [`video-standard-definition`](https://pictogrammers.com/library/mdi/icon/video-standard-definition/) icon.
-}
videoStandardDefinition : IconShape
videoStandardDefinition =
    iconShape "M14 14V10C14 9.7 13.8 9.5 13.5 9.5H12.5V14.5H13.5C13.8 14.5 14 14.3 14 14M17 7V10.5L21 6.5V17.5L17 13.5V17C17 17.6 16.6 18 16 18H4C3.4 18 3 17.6 3 17V7C3 6.4 3.4 6 4 6H16C16.6 6 17 6.4 17 7M14 16C14.8 16 15.5 15.3 15.5 14.5V9.5C15.5 8.7 14.8 8 14 8H11V16H14M9 8H5.5C4.67 8 4 8.67 4 9.5V11C4 11.83 4.67 12.5 5.5 12.5H7.5V14.5H4V16H7.5C8.33 16 9 15.33 9 14.5V12.5C9 11.67 8.33 11 7.5 11H5.5V9.5H9V8Z"


{-| The [`video-switch`](https://pictogrammers.com/library/mdi/icon/video-switch/) icon.
-}
videoSwitch : IconShape
videoSwitch =
    iconShape "M13,15.5V13H7V15.5L3.5,12L7,8.5V11H13V8.5L16.5,12M18,9.5V6A1,1 0 0,0 17,5H3A1,1 0 0,0 2,6V18A1,1 0 0,0 3,19H17A1,1 0 0,0 18,18V14.5L22,18.5V5.5L18,9.5Z"


{-| The [`video-switch-outline`](https://pictogrammers.com/library/mdi/icon/video-switch-outline/) icon.
-}
videoSwitchOutline : IconShape
videoSwitchOutline =
    iconShape "M8 13H12V15L15 12L12 9V11H8V9L5 12L8 15V13M18 9.5V6C18 5.4 17.5 5 17 5H3C2.5 5 2 5.4 2 6V18C2 18.5 2.5 19 3 19H17C17.5 19 18 18.5 18 18V14.5L22 18.5V5.5L18 9.5M16 17H4V7H16V17Z"


{-| The [`video-vintage`](https://pictogrammers.com/library/mdi/icon/video-vintage/) icon.
-}
videoVintage : IconShape
videoVintage =
    iconShape "M18,14.5V11A1,1 0 0,0 17,10H16C18.24,8.39 18.76,5.27 17.15,3C15.54,0.78 12.42,0.26 10.17,1.87C9.5,2.35 8.96,3 8.6,3.73C6.25,2.28 3.17,3 1.72,5.37C0.28,7.72 1,10.8 3.36,12.25C3.57,12.37 3.78,12.5 4,12.58V21A1,1 0 0,0 5,22H17A1,1 0 0,0 18,21V17.5L22,21.5V10.5L18,14.5M13,4A2,2 0 0,1 15,6A2,2 0 0,1 13,8A2,2 0 0,1 11,6A2,2 0 0,1 13,4M6,6A2,2 0 0,1 8,8A2,2 0 0,1 6,10A2,2 0 0,1 4,8A2,2 0 0,1 6,6Z"


{-| The [`video-wireless`](https://pictogrammers.com/library/mdi/icon/video-wireless/) icon.
-}
videoWireless : IconShape
videoWireless =
    iconShape "M17,14.5V11A1,1 0 0,0 16,10H4A1,1 0 0,0 3,11V21A1,1 0 0,0 4,22H16A1,1 0 0,0 17,21V17.5L21,21.5V10.5M3,3.86L4.4,5.24C7.5,2.19 12.5,2.19 15.6,5.24L17,3.86C13.14,0.05 6.87,0.05 3,3.86M5.8,6.62L7.2,8C8.75,6.5 11.25,6.5 12.8,8L14.2,6.62C11.88,4.34 8.12,4.34 5.8,6.62Z"


{-| The [`video-wireless-outline`](https://pictogrammers.com/library/mdi/icon/video-wireless-outline/) icon.
-}
videoWirelessOutline : IconShape
videoWirelessOutline =
    iconShape "M15,12V20H5V12H15M16,10H4A1,1 0 0,0 3,11V21A1,1 0 0,0 4,22H16A1,1 0 0,0 17,21V17.5L21,21.5V10.5L17,14.5V11A1,1 0 0,0 16,10M3,3.86L4.4,5.24C7.5,2.19 12.5,2.19 15.6,5.24L17,3.86C13.14,0.05 6.87,0.05 3,3.86M5.8,6.63L7.2,8C8.75,6.5 11.25,6.5 12.8,8L14.2,6.63C11.88,4.34 8.12,4.34 5.8,6.63Z"


{-| The [`view-agenda`](https://pictogrammers.com/library/mdi/icon/view-agenda/) icon.
-}
viewAgenda : IconShape
viewAgenda =
    iconShape "M21 3H3C2.4 3 2 3.4 2 4V10C2 10.6 2.4 11 3 11H21C21.6 11 22 10.6 22 10V4C22 3.4 21.6 3 21 3M21 13H3C2.4 13 2 13.4 2 14V20C2 20.6 2.4 21 3 21H21C21.6 21 22 20.6 22 20V14C22 13.4 21.6 13 21 13Z"


{-| The [`view-agenda-outline`](https://pictogrammers.com/library/mdi/icon/view-agenda-outline/) icon.
-}
viewAgendaOutline : IconShape
viewAgendaOutline =
    iconShape "M21 13H3A1 1 0 0 0 2 14V20A1 1 0 0 0 3 21H21A1 1 0 0 0 22 20V14A1 1 0 0 0 21 13M20 19H4V15H20M21 3H3A1 1 0 0 0 2 4V10A1 1 0 0 0 3 11H21A1 1 0 0 0 22 10V4A1 1 0 0 0 21 3M20 9H4V5H20Z"


{-| The [`view-array`](https://pictogrammers.com/library/mdi/icon/view-array/) icon.
-}
viewArray : IconShape
viewArray =
    iconShape "M8,18H17V5H8M18,5V18H21V5M4,18H7V5H4V18Z"


{-| The [`view-array-outline`](https://pictogrammers.com/library/mdi/icon/view-array-outline/) icon.
-}
viewArrayOutline : IconShape
viewArrayOutline =
    iconShape "M15 7V16H10V7H15M21 5H18V18H21V5M17 5H8V18H17V5M7 5H4V18H7V5Z"


{-| The [`view-carousel`](https://pictogrammers.com/library/mdi/icon/view-carousel/) icon.
-}
viewCarousel : IconShape
viewCarousel =
    iconShape "M18,6V17H22V6M2,17H6V6H2M7,19H17V4H7V19Z"


{-| The [`view-carousel-outline`](https://pictogrammers.com/library/mdi/icon/view-carousel-outline/) icon.
-}
viewCarouselOutline : IconShape
viewCarouselOutline =
    iconShape "M2 6H6V17H2V6M7 19H17V4H7V19M9 6H15V17H9V6M18 6H22V17H18V6Z"


{-| The [`view-column`](https://pictogrammers.com/library/mdi/icon/view-column/) icon.
-}
viewColumn : IconShape
viewColumn =
    iconShape "M16,5V18H21V5M4,18H9V5H4M10,18H15V5H10V18Z"


{-| The [`view-column-outline`](https://pictogrammers.com/library/mdi/icon/view-column-outline/) icon.
-}
viewColumnOutline : IconShape
viewColumnOutline =
    iconShape "M4 5V18H21V5H4M14 7V16H11V7H14M6 7H9V16H6V7M19 16H16V7H19V16Z"


{-| The [`view-comfy`](https://pictogrammers.com/library/mdi/icon/view-comfy/) icon.
-}
viewComfy : IconShape
viewComfy =
    iconShape "M3,9H7V5H3V9M3,14H7V10H3V14M8,14H12V10H8V14M13,14H17V10H13V14M8,9H12V5H8V9M13,5V9H17V5H13M18,14H22V10H18V14M3,19H7V15H3V19M8,19H12V15H8V19M13,19H17V15H13V19M18,19H22V15H18V19M18,5V9H22V5H18Z"


{-| The [`view-comfy-outline`](https://pictogrammers.com/library/mdi/icon/view-comfy-outline/) icon.
-}
viewComfyOutline : IconShape
viewComfyOutline =
    iconShape "M3 5V19H22V5H3M20 9H17.75V7H20V9M9.25 11H11.5V13H9.25V11M7.25 13H5V11H7.25V13M11.5 9H9.25V7H11.5V9M13.5 7H15.75V9H13.5V7M11.5 15V17H9.25V15H11.5M13.5 15H15.75V17H13.5V15M13.5 13V11H15.75V13H13.5M17.75 11H20V13H17.75V11M7.25 7V9H5V7H7.25M5 15H7.25V17H5V15M17.75 17V15H20V17H17.75Z"


{-| The [`view-compact`](https://pictogrammers.com/library/mdi/icon/view-compact/) icon.
-}
viewCompact : IconShape
viewCompact =
    iconShape "M3,19H9V12H3V19M10,19H22V12H10V19M3,5V11H22V5H3Z"


{-| The [`view-compact-outline`](https://pictogrammers.com/library/mdi/icon/view-compact-outline/) icon.
-}
viewCompactOutline : IconShape
viewCompactOutline =
    iconShape "M3,5V19H22V5H3M5,7H20V11H5V7M5,17V13H9V17H5M11,17V13H20V17H11Z"


{-| The [`view-dashboard`](https://pictogrammers.com/library/mdi/icon/view-dashboard/) icon.
-}
viewDashboard : IconShape
viewDashboard =
    iconShape "M13,3V9H21V3M13,21H21V11H13M3,21H11V15H3M3,13H11V3H3V13Z"


{-| The [`view-dashboard-edit`](https://pictogrammers.com/library/mdi/icon/view-dashboard-edit/) icon.
-}
viewDashboardEdit : IconShape
viewDashboardEdit =
    iconShape "M21 13.1C20.9 13.1 20.7 13.2 20.6 13.3L19.6 14.3L21.7 16.4L22.7 15.4C22.9 15.2 22.9 14.8 22.7 14.6L21.4 13.3C21.3 13.2 21.2 13.1 21 13.1M19.1 14.9L13 20.9V23H15.1L21.2 16.9L19.1 14.9M21 9H13V3H21V9M13 18.06V11H21V11.1C20.24 11.1 19.57 11.5 19.19 11.89L13 18.06M11 13H3V3H11V13M11 21H3V15H11V21Z"


{-| The [`view-dashboard-edit-outline`](https://pictogrammers.com/library/mdi/icon/view-dashboard-edit-outline/) icon.
-}
viewDashboardEditOutline : IconShape
viewDashboardEditOutline =
    iconShape "M21 13.1C20.9 13.1 20.7 13.2 20.6 13.3L19.6 14.3L21.7 16.4L22.7 15.4C22.9 15.2 22.9 14.8 22.7 14.6L21.4 13.3C21.3 13.2 21.2 13.1 21 13.1M19.1 14.9L13 20.9V23H15.1L21.2 16.9L19.1 14.9M21 3H13V9H21V3M19 7H15V5H19V7M13 18.06V11H21V11.1C20.24 11.1 19.57 11.5 19.19 11.89L18.07 13H15V16.07L13 18.06M11 3H3V13H11V3M9 11H5V5H9V11M11 20.06V15H3V21H11V20.06M9 19H5V17H9V19Z"


{-| The [`view-dashboard-outline`](https://pictogrammers.com/library/mdi/icon/view-dashboard-outline/) icon.
-}
viewDashboardOutline : IconShape
viewDashboardOutline =
    iconShape "M19,5V7H15V5H19M9,5V11H5V5H9M19,13V19H15V13H19M9,17V19H5V17H9M21,3H13V9H21V3M11,3H3V13H11V3M21,11H13V21H21V11M11,15H3V21H11V15Z"


{-| The [`view-dashboard-variant`](https://pictogrammers.com/library/mdi/icon/view-dashboard-variant/) icon.
-}
viewDashboardVariant : IconShape
viewDashboardVariant =
    iconShape "M2,5V19H8V5H2M9,5V10H15V5H9M16,5V14H22V5H16M9,11V19H15V11H9M16,15V19H22V15H16Z"


{-| The [`view-dashboard-variant-outline`](https://pictogrammers.com/library/mdi/icon/view-dashboard-variant-outline/) icon.
-}
viewDashboardVariantOutline : IconShape
viewDashboardVariantOutline =
    iconShape "M2 5V19H22V5H2M20 12H16V7H20V12M14 10H10V7H14V10M10 12H14V17H10V12M4 7H8V17H4V7M16 17V14H20V17H16Z"


{-| The [`view-day`](https://pictogrammers.com/library/mdi/icon/view-day/) icon.
-}
viewDay : IconShape
viewDay =
    iconShape "M2,3V6H21V3M20,8H3A1,1 0 0,0 2,9V15A1,1 0 0,0 3,16H20A1,1 0 0,0 21,15V9A1,1 0 0,0 20,8M2,21H21V18H2V21Z"


{-| The [`view-day-outline`](https://pictogrammers.com/library/mdi/icon/view-day-outline/) icon.
-}
viewDayOutline : IconShape
viewDayOutline =
    iconShape "M21 18H2V20H21V18M19 10V14H4V10H19M20 8H3C2.45 8 2 8.45 2 9V15C2 15.55 2.45 16 3 16H20C20.55 16 21 15.55 21 15V9C21 8.45 20.55 8 20 8M21 4H2V6H21V4Z"


{-| The [`view-gallery`](https://pictogrammers.com/library/mdi/icon/view-gallery/) icon.
-}
viewGallery : IconShape
viewGallery =
    iconShape "M21 3H2V16H21V3M2 17H6V21H2V17M7 17H11V21H7V17M12 17H16V21H12V17M17 17H21V21H17V17Z"


{-| The [`view-gallery-outline`](https://pictogrammers.com/library/mdi/icon/view-gallery-outline/) icon.
-}
viewGalleryOutline : IconShape
viewGalleryOutline =
    iconShape "M1 3V21H23V3H1M21 5V14H3V5H21M11 16V19H8V16H11M3 16H6V19H3V16M13 19V16H16V19H13M18 19V16H21V19H18Z"


{-| The [`view-grid`](https://pictogrammers.com/library/mdi/icon/view-grid/) icon.
-}
viewGrid : IconShape
viewGrid =
    iconShape "M3,11H11V3H3M3,21H11V13H3M13,21H21V13H13M13,3V11H21V3"


{-| The [`view-grid-compact`](https://pictogrammers.com/library/mdi/icon/view-grid-compact/) icon.
-}
viewGridCompact : IconShape
viewGridCompact =
    iconShape "M2 5H4V7H2V5M5 5H7V7H5V5M8 5H10V7H8V5M11 5H13V7H11V5M14 5H16V7H14V5M17 5H19V7H17V5M20 5H22V7H20V5M2 8H4V10H2V8M5 8H7V10H5V8M8 8H10V10H8V8M11 8H13V10H11V8M14 8H16V10H14V8M17 8H19V10H17V8M20 8H22V10H20V8M2 11H4V13H2V11M5 11H7V13H5V11M8 11H10V13H8V11M11 11H13V13H11V11M14 11H16V13H14V11M17 11H19V13H17V11M20 11H22V13H20V11M2 14H4V16H2V14M5 14H7V16H5V14M8 14H10V16H8V14M11 14H13V16H11V14M14 14H16V16H14V14M17 14H19V16H17V14M20 14H22V16H20V14M2 17H4V19H2V17M5 17H7V19H5V17M8 17H10V19H8V17M11 17H13V19H11V17M14 17H16V19H14V17M17 17H19V19H17V17M20 17H22V19H20V17"


{-| The [`view-grid-outline`](https://pictogrammers.com/library/mdi/icon/view-grid-outline/) icon.
-}
viewGridOutline : IconShape
viewGridOutline =
    iconShape "M3 11H11V3H3M5 5H9V9H5M13 21H21V13H13M15 15H19V19H15M3 21H11V13H3M5 15H9V19H5M13 3V11H21V3M19 9H15V5H19Z"


{-| The [`view-grid-plus`](https://pictogrammers.com/library/mdi/icon/view-grid-plus/) icon.
-}
viewGridPlus : IconShape
viewGridPlus =
    iconShape "M13 3V11H21V3H13M3 21H11V13H3V21M3 3V11H11V3H3M13 16H16V13H18V16H21V18H18V21H16V18H13V16Z"


{-| The [`view-grid-plus-outline`](https://pictogrammers.com/library/mdi/icon/view-grid-plus-outline/) icon.
-}
viewGridPlusOutline : IconShape
viewGridPlusOutline =
    iconShape "M3 21H11V13H3M5 15H9V19H5M3 11H11V3H3M5 5H9V9H5M13 3V11H21V3M19 9H15V5H19M18 16H21V18H18V21H16V18H13V16H16V13H18Z"


{-| The [`view-headline`](https://pictogrammers.com/library/mdi/icon/view-headline/) icon.
-}
viewHeadline : IconShape
viewHeadline =
    iconShape "M4,5V7H21V5M4,11H21V9H4M4,19H21V17H4M4,15H21V13H4V15Z"


{-| The [`view-list`](https://pictogrammers.com/library/mdi/icon/view-list/) icon.
-}
viewList : IconShape
viewList =
    iconShape "M9,5V9H21V5M9,19H21V15H9M9,14H21V10H9M4,9H8V5H4M4,19H8V15H4M4,14H8V10H4V14Z"


{-| The [`view-list-outline`](https://pictogrammers.com/library/mdi/icon/view-list-outline/) icon.
-}
viewListOutline : IconShape
viewListOutline =
    iconShape "M3 5V19H20V5H3M7 7V9H5V7H7M5 13V11H7V13H5M5 15H7V17H5V15M18 17H9V15H18V17M18 13H9V11H18V13M18 9H9V7H18V9Z"


{-| The [`view-module`](https://pictogrammers.com/library/mdi/icon/view-module/) icon.
-}
viewModule : IconShape
viewModule =
    iconShape "M16,5V11H21V5M10,11H15V5H10M16,18H21V12H16M10,18H15V12H10M4,18H9V12H4M4,11H9V5H4V11Z"


{-| The [`view-module-outline`](https://pictogrammers.com/library/mdi/icon/view-module-outline/) icon.
-}
viewModuleOutline : IconShape
viewModuleOutline =
    iconShape "M4 5V18H21V5H4M14 7V10.5H11V7H14M6 7H9V10.5H6V7M6 16V12.5H9V16H6M11 16V12.5H14V16H11M19 16H16V12.5H19V16M16 10.5V7H19V10.5H16Z"


{-| The [`view-parallel`](https://pictogrammers.com/library/mdi/icon/view-parallel/) icon.
-}
viewParallel : IconShape
viewParallel =
    iconShape "M19 3V21H15V3H19M14 3V21H10V3H14M9 3V21H5V3H9Z"


{-| The [`view-parallel-outline`](https://pictogrammers.com/library/mdi/icon/view-parallel-outline/) icon.
-}
viewParallelOutline : IconShape
viewParallelOutline =
    iconShape "M19 3H5V21H19V3M17 19H15V5H17V19M13 19H11V5H13V19M7 5H9V19H7V5Z"


{-| The [`view-quilt`](https://pictogrammers.com/library/mdi/icon/view-quilt/) icon.
-}
viewQuilt : IconShape
viewQuilt =
    iconShape "M10,5V11H21V5M16,18H21V12H16M4,18H9V5H4M10,18H15V12H10V18Z"


{-| The [`view-quilt-outline`](https://pictogrammers.com/library/mdi/icon/view-quilt-outline/) icon.
-}
viewQuiltOutline : IconShape
viewQuiltOutline =
    iconShape "M4 5V18H21V5H4M6 16V7H9V16H6M11 16V12.5H14V16H11M19 16H16V12.5H19V16M11 10.5V7H19V10.5H11Z"


{-| The [`view-sequential`](https://pictogrammers.com/library/mdi/icon/view-sequential/) icon.
-}
viewSequential : IconShape
viewSequential =
    iconShape "M3 5H21V9H3V5M3 10H21V14H3V10M3 15H21V19H3V15Z"


{-| The [`view-sequential-outline`](https://pictogrammers.com/library/mdi/icon/view-sequential-outline/) icon.
-}
viewSequentialOutline : IconShape
viewSequentialOutline =
    iconShape "M3 5V19H21V5H3M19 7V9H5V7H19M19 11V13H5V11H19M5 17V15H19V17H5Z"


{-| The [`view-split-horizontal`](https://pictogrammers.com/library/mdi/icon/view-split-horizontal/) icon.
-}
viewSplitHorizontal : IconShape
viewSplitHorizontal =
    iconShape "M3,5H21V7H3V5M3,11V9H21V11H3M3,19V13H21V19H3Z"


{-| The [`view-split-vertical`](https://pictogrammers.com/library/mdi/icon/view-split-vertical/) icon.
-}
viewSplitVertical : IconShape
viewSplitVertical =
    iconShape "M13,5H21V19H13V5M3,5H11V7H3V5M3,11V9H11V11H3M3,19V17H11V19H3M3,15V13H11V15H3Z"


{-| The [`view-stream`](https://pictogrammers.com/library/mdi/icon/view-stream/) icon.
-}
viewStream : IconShape
viewStream =
    iconShape "M4,5V11H21V5M4,18H21V12H4V18Z"


{-| The [`view-stream-outline`](https://pictogrammers.com/library/mdi/icon/view-stream-outline/) icon.
-}
viewStreamOutline : IconShape
viewStreamOutline =
    iconShape "M4 6V18H21V6H4M19 16H6V13H19V16M6 11V8H19V11H6Z"


{-| The [`view-week`](https://pictogrammers.com/library/mdi/icon/view-week/) icon.
-}
viewWeek : IconShape
viewWeek =
    iconShape "M13,5H10A1,1 0 0,0 9,6V18A1,1 0 0,0 10,19H13A1,1 0 0,0 14,18V6A1,1 0 0,0 13,5M20,5H17A1,1 0 0,0 16,6V18A1,1 0 0,0 17,19H20A1,1 0 0,0 21,18V6A1,1 0 0,0 20,5M6,5H3A1,1 0 0,0 2,6V18A1,1 0 0,0 3,19H6A1,1 0 0,0 7,18V6A1,1 0 0,0 6,5Z"


{-| The [`view-week-outline`](https://pictogrammers.com/library/mdi/icon/view-week-outline/) icon.
-}
viewWeekOutline : IconShape
viewWeekOutline =
    iconShape "M21 4H3C2.45 4 2 4.45 2 5V19C2 19.55 2.45 20 3 20H21C21.55 20 22 19.55 22 19V5C22 4.45 21.55 4 21 4M8 18H4V6H8V18M14 18H10V6H14V18M20 18H16V6H20V18Z"


{-| The [`violin`](https://pictogrammers.com/library/mdi/icon/violin/) icon.
-}
violin : IconShape
violin =
    iconShape "M11,2A1,1 0 0,0 10,3V5L10,9A0.5,0.5 0 0,0 10.5,9.5H12A0.5,0.5 0 0,1 12.5,10A0.5,0.5 0 0,1 12,10.5H10.5C9.73,10.5 9,9.77 9,9V5.16C7.27,5.6 6,7.13 6,9V10.5A2.5,2.5 0 0,1 8.5,13A2.5,2.5 0 0,1 6,15.5V17C6,19.77 8.23,22 11,22H13C15.77,22 18,19.77 18,17V15.5A2.5,2.5 0 0,1 15.5,13A2.5,2.5 0 0,1 18,10.5V9C18,6.78 16.22,5 14,5V3A1,1 0 0,0 13,2H11M10.75,16.5H13.25L12.75,20H11.25L10.75,16.5Z"


{-| The [`virtual-reality`](https://pictogrammers.com/library/mdi/icon/virtual-reality/) icon.
-}
virtualReality : IconShape
virtualReality =
    iconShape "M5,3C3.89,3 3,3.9 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19V5A2,2 0 0,0 19,3H5M6,9H7.5L8.5,12.43L9.5,9H11L9.25,15H7.75L6,9M13,9H16.5C17.35,9 18,9.65 18,10.5V11.5C18,12.1 17.6,12.65 17.1,12.9L18,15H16.5L15.65,13H14.5V15H13V9M14.5,10.5V11.5H16.5V10.5H14.5Z"


{-| The [`virus`](https://pictogrammers.com/library/mdi/icon/virus/) icon.
-}
virus : IconShape
virus =
    iconShape "M19.82 14C20.13 14.45 20.66 14.75 21.25 14.75C22.22 14.75 23 13.97 23 13S22.22 11.25 21.25 11.25C20.66 11.25 20.13 11.55 19.82 12H19C19 10.43 18.5 9 17.6 7.81L18.94 6.47C19.5 6.57 20.07 6.41 20.5 6C21.17 5.31 21.17 4.2 20.5 3.5C19.81 2.83 18.7 2.83 18 3.5C17.59 3.93 17.43 4.5 17.53 5.06L16.19 6.4C15.27 5.71 14.19 5.25 13 5.08V3.68C13.45 3.37 13.75 2.84 13.75 2.25C13.75 1.28 12.97 .5 12 .5S10.25 1.28 10.25 2.25C10.25 2.84 10.55 3.37 11 3.68V5.08C10.1 5.21 9.26 5.5 8.5 5.94L7.39 4.35C7.58 3.83 7.53 3.23 7.19 2.75C6.63 1.96 5.54 1.76 4.75 2.32C3.96 2.87 3.76 3.96 4.32 4.75C4.66 5.24 5.2 5.5 5.75 5.5L6.93 7.18C6.5 7.61 6.16 8.09 5.87 8.62C5.25 8.38 4.5 8.5 4 9C3.33 9.7 3.33 10.8 4 11.5C4.29 11.77 4.64 11.93 5 12L5 12C5 12.54 5.07 13.06 5.18 13.56L3.87 13.91C3.45 13.56 2.87 13.41 2.29 13.56C1.36 13.81 .808 14.77 1.06 15.71C1.31 16.64 2.28 17.19 3.21 16.94C3.78 16.78 4.21 16.36 4.39 15.84L5.9 15.43C6.35 16.22 6.95 16.92 7.65 17.5L6.55 19.5C6 19.58 5.5 19.89 5.21 20.42C4.75 21.27 5.07 22.33 5.92 22.79C6.77 23.25 7.83 22.93 8.29 22.08C8.57 21.56 8.56 20.96 8.31 20.47L9.38 18.5C10.19 18.82 11.07 19 12 19C12.06 19 12.12 19 12.18 19C12.05 19.26 12 19.56 12 19.88C12.08 20.85 12.92 21.57 13.88 21.5S15.57 20.58 15.5 19.62C15.46 19.12 15.21 18.68 14.85 18.39C15.32 18.18 15.77 17.91 16.19 17.6L18.53 19.94C18.43 20.5 18.59 21.07 19 21.5C19.7 22.17 20.8 22.17 21.5 21.5S22.17 19.7 21.5 19C21.07 18.59 20.5 18.43 19.94 18.53L17.6 16.19C18.09 15.54 18.47 14.8 18.71 14H19.82M10.5 12C9.67 12 9 11.33 9 10.5S9.67 9 10.5 9 12 9.67 12 10.5 11.33 12 10.5 12M14 15C13.45 15 13 14.55 13 14C13 13.45 13.45 13 14 13S15 13.45 15 14C15 14.55 14.55 15 14 15Z"


{-| The [`virus-off`](https://pictogrammers.com/library/mdi/icon/virus-off/) icon.
-}
virusOff : IconShape
virusOff =
    iconShape "M7.42 4.22L5.28 2.08C5.97 1.88 6.75 2.12 7.19 2.75C7.5 3.19 7.56 3.73 7.42 4.22M21.25 11.25C20.66 11.25 20.13 11.55 19.82 12H19C19 10.43 18.5 9 17.6 7.81L18.94 6.47C19.5 6.57 20.07 6.41 20.5 6C21.17 5.31 21.17 4.2 20.5 3.5C19.81 2.83 18.7 2.83 18 3.5C17.59 3.93 17.43 4.5 17.53 5.06L16.19 6.4C15.27 5.71 14.19 5.25 13 5.08V3.68C13.45 3.37 13.75 2.84 13.75 2.25C13.75 1.28 12.97 .5 12 .5S10.25 1.28 10.25 2.25C10.25 2.84 10.55 3.37 11 3.68V5.08C10.26 5.19 9.57 5.41 8.92 5.72L18.28 15.08C18.45 14.73 18.6 14.38 18.71 14H19.82C20.13 14.45 20.66 14.75 21.25 14.75C22.22 14.75 23 13.97 23 13S22.22 11.25 21.25 11.25M22.11 21.46L20.84 22.73L20.1 22C19.7 21.96 19.31 21.8 19 21.5C18.7 21.19 18.54 20.8 18.5 20.4L15.9 17.79C15.57 18 15.22 18.23 14.85 18.39C15.21 18.68 15.46 19.12 15.5 19.62C15.57 20.58 14.84 21.43 13.88 21.5S12.08 20.85 12 19.88C12 19.56 12.05 19.26 12.18 19H12C11.07 19 10.19 18.82 9.38 18.5L8.31 20.47C8.56 20.96 8.57 21.56 8.29 22.08C7.83 22.93 6.77 23.25 5.92 22.79C5.07 22.33 4.75 21.27 5.21 20.42C5.5 19.89 6 19.58 6.55 19.5L7.65 17.5C6.95 16.92 6.35 16.22 5.9 15.43L4.39 15.84C4.21 16.36 3.78 16.78 3.21 16.94C2.28 17.19 1.31 16.64 1.06 15.71C.808 14.77 1.36 13.81 2.29 13.56C2.87 13.41 3.45 13.56 3.87 13.91L5.18 13.56C5.07 13.06 5 12.54 5 12C4.64 11.93 4.29 11.77 4 11.5C3.33 10.8 3.33 9.7 4 9C4.5 8.5 5.25 8.38 5.87 8.62C5.97 8.44 6.09 8.27 6.2 8.09L1.11 3L2.39 1.73L22.11 21.46Z"


{-| The [`virus-off-outline`](https://pictogrammers.com/library/mdi/icon/virus-off-outline/) icon.
-}
virusOffOutline : IconShape
virusOffOutline =
    iconShape "M23 13C23 13.97 22.22 14.75 21.25 14.75C20.66 14.75 20.13 14.45 19.82 14H18.71C18.6 14.38 18.45 14.73 18.28 15.08L16.73 13.53C16.89 13.05 17 12.54 17 12C17 9.24 14.76 7 12 7C11.46 7 10.95 7.11 10.47 7.27L8.92 5.72C9.57 5.41 10.26 5.19 11 5.08V3.68C10.55 3.37 10.25 2.84 10.25 2.25C10.25 1.28 11.03 .5 12 .5S13.75 1.28 13.75 2.25C13.75 2.84 13.45 3.37 13 3.68V5.08C14.19 5.25 15.27 5.71 16.19 6.4L17.53 5.06C17.43 4.5 17.59 3.93 18 3.5C18.35 3.17 18.8 3 19.25 3S20.15 3.17 20.5 3.5C21.17 4.2 21.17 5.31 20.5 6C20.15 6.33 19.7 6.5 19.25 6.5C19.15 6.5 19.05 6.5 18.94 6.47L17.6 7.81C18.5 9 19 10.43 19 12H19.82C20.13 11.55 20.66 11.25 21.25 11.25C22.22 11.25 23 12.03 23 13M22.11 21.46L20.84 22.73L20.1 22C19.7 21.95 19.31 21.79 19 21.5C18.7 21.19 18.54 20.8 18.5 20.4L15.9 17.79C15.57 18 15.22 18.23 14.85 18.39C15.21 18.68 15.46 19.12 15.5 19.62C15.57 20.58 14.85 21.42 13.88 21.5H13.75C12.84 21.5 12.08 20.8 12 19.88C12 19.56 12.05 19.26 12.18 19H12C11.07 19 10.19 18.82 9.38 18.5L8.31 20.47C8.56 20.96 8.57 21.56 8.29 22.08C7.97 22.67 7.37 23 6.75 23C6.47 23 6.18 22.93 5.92 22.79C5.07 22.33 4.75 21.27 5.21 20.42C5.5 19.89 6 19.58 6.55 19.5L7.65 17.5C6.95 16.92 6.35 16.22 5.9 15.43L4.39 15.84C4.21 16.36 3.78 16.78 3.21 16.94C3.05 17 2.9 17 2.75 17C2 17 1.27 16.5 1.06 15.71C.809 14.77 1.36 13.81 2.29 13.56C2.44 13.5 2.6 13.5 2.75 13.5C3.16 13.5 3.56 13.65 3.87 13.91L5.18 13.56C5.07 13.06 5 12.54 5 12C4.64 11.92 4.29 11.77 4 11.5C3.33 10.8 3.33 9.7 4 9C4.35 8.67 4.8 8.5 5.25 8.5C5.46 8.5 5.67 8.54 5.87 8.62C5.97 8.44 6.09 8.27 6.2 8.09L1.11 3L2.39 1.73L22.11 21.46M14.45 16.34L7.66 9.55C7.25 10.28 7 11.11 7 12C7 14.76 9.24 17 12 17C12.9 17 13.73 16.75 14.45 16.34M7.19 2.75C6.84 2.26 6.3 2 5.75 2C5.59 2 5.43 2.03 5.27 2.07L7.42 4.22C7.56 3.73 7.5 3.19 7.19 2.75Z"


{-| The [`virus-outline`](https://pictogrammers.com/library/mdi/icon/virus-outline/) icon.
-}
virusOutline : IconShape
virusOutline =
    iconShape "M12 .5C11.03 .5 10.25 1.28 10.25 2.25C10.25 2.84 10.55 3.37 11 3.68V5.08C10.1 5.21 9.26 5.5 8.5 5.94L7.39 4.35C7.58 3.83 7.53 3.23 7.19 2.75C6.84 2.26 6.3 2 5.75 2C5.4 2 5.05 2.1 4.75 2.32C3.96 2.87 3.76 3.96 4.32 4.75C4.66 5.24 5.2 5.5 5.75 5.5L6.93 7.18C6.5 7.61 6.16 8.09 5.87 8.62C5.67 8.54 5.46 8.5 5.25 8.5C4.8 8.5 4.35 8.67 4 9C3.33 9.7 3.33 10.8 4 11.5C4.29 11.77 4.64 11.92 5 12L5 12C5 12.54 5.07 13.06 5.18 13.56L3.87 13.91C3.56 13.65 3.16 13.5 2.75 13.5C2.6 13.5 2.44 13.5 2.29 13.56C1.36 13.81 .809 14.77 1.06 15.71C1.27 16.5 2 17 2.75 17C2.9 17 3.05 17 3.21 16.94C3.78 16.78 4.21 16.36 4.39 15.84L5.9 15.43C6.35 16.22 6.95 16.92 7.65 17.5L6.55 19.5C6 19.58 5.5 19.89 5.21 20.42C4.75 21.27 5.07 22.33 5.92 22.79C6.18 22.93 6.47 23 6.75 23C7.37 23 7.97 22.67 8.29 22.08C8.57 21.56 8.56 20.96 8.31 20.47L9.38 18.5C10.19 18.82 11.07 19 12 19C12.06 19 12.12 19 12.18 19C12.05 19.26 12 19.56 12 19.88C12.08 20.8 12.84 21.5 13.75 21.5C13.79 21.5 13.84 21.5 13.88 21.5C14.85 21.42 15.57 20.58 15.5 19.62C15.46 19.12 15.21 18.68 14.85 18.39C15.32 18.18 15.77 17.91 16.19 17.6L18.53 19.94C18.43 20.5 18.59 21.07 19 21.5C19.35 21.83 19.8 22 20.25 22S21.15 21.83 21.5 21.5C22.17 20.8 22.17 19.7 21.5 19C21.15 18.67 20.7 18.5 20.25 18.5C20.15 18.5 20.05 18.5 19.94 18.53L17.6 16.19C18.09 15.54 18.47 14.8 18.71 14H19.82C20.13 14.45 20.66 14.75 21.25 14.75C22.22 14.75 23 13.97 23 13S22.22 11.25 21.25 11.25C20.66 11.25 20.13 11.55 19.82 12H19C19 10.43 18.5 9 17.6 7.81L18.94 6.47C19.05 6.5 19.15 6.5 19.25 6.5C19.7 6.5 20.15 6.33 20.5 6C21.17 5.31 21.17 4.2 20.5 3.5C20.15 3.17 19.7 3 19.25 3S18.35 3.17 18 3.5C17.59 3.93 17.43 4.5 17.53 5.06L16.19 6.4C15.27 5.71 14.19 5.25 13 5.08V3.68C13.45 3.37 13.75 2.84 13.75 2.25C13.75 1.28 12.97 .5 12 .5M12 17C9.24 17 7 14.76 7 12S9.24 7 12 7 17 9.24 17 12 14.76 17 12 17M10.5 9C9.67 9 9 9.67 9 10.5S9.67 12 10.5 12 12 11.33 12 10.5 11.33 9 10.5 9M14 13C13.45 13 13 13.45 13 14C13 14.55 13.45 15 14 15C14.55 15 15 14.55 15 14C15 13.45 14.55 13 14 13Z"


{-| The [`voicemail`](https://pictogrammers.com/library/mdi/icon/voicemail/) icon.
-}
voicemail : IconShape
voicemail =
    iconShape "M18.5,15A3.5,3.5 0 0,1 15,11.5A3.5,3.5 0 0,1 18.5,8A3.5,3.5 0 0,1 22,11.5A3.5,3.5 0 0,1 18.5,15M5.5,15A3.5,3.5 0 0,1 2,11.5A3.5,3.5 0 0,1 5.5,8A3.5,3.5 0 0,1 9,11.5A3.5,3.5 0 0,1 5.5,15M18.5,6A5.5,5.5 0 0,0 13,11.5C13,12.83 13.47,14.05 14.26,15H9.74C10.53,14.05 11,12.83 11,11.5A5.5,5.5 0 0,0 5.5,6A5.5,5.5 0 0,0 0,11.5A5.5,5.5 0 0,0 5.5,17H18.5A5.5,5.5 0 0,0 24,11.5A5.5,5.5 0 0,0 18.5,6Z"


{-| The [`volcano`](https://pictogrammers.com/library/mdi/icon/volcano/) icon.
-}
volcano : IconShape
volcano =
    iconShape "M18 8H11L9 13H6L2 22H22L18 8M13 1H15V5H13V1M16.12 5.47L18.95 2.64L20.36 4.05L17.54 6.88L16.12 5.47M7.64 4.05L9.05 2.64L11.88 5.46L10.47 6.88L7.64 4.05Z"


{-| The [`volcano-outline`](https://pictogrammers.com/library/mdi/icon/volcano-outline/) icon.
-}
volcanoOutline : IconShape
volcanoOutline =
    iconShape "M18 8H11L9 13H6L2 22H22L18 8M7.3 15H10.35L10.85 13.74L12.35 10H16.5L19.35 20H5.08L7.3 15M13 1H15V5H13V1M16.12 5.47L18.95 2.64L20.36 4.05L17.54 6.88L16.12 5.47M7.64 4.05L9.05 2.64L11.88 5.46L10.47 6.88L7.64 4.05Z"


{-| The [`volleyball`](https://pictogrammers.com/library/mdi/icon/volleyball/) icon.
-}
volleyball : IconShape
volleyball =
    iconShape "M19.04 4.85C17.34 3.2 15.33 2.25 13 2V5.62L22 10.8C21.72 8.5 20.73 6.5 19.04 4.85M12 22C15.44 22 18.16 20.62 20.17 17.86L17.06 16L8.07 21.2C9.32 21.73 10.64 22 12 22M13 11.41L21.15 16.07C21.59 15.13 21.88 14.14 22 13.11L13 7.93V11.41M3.88 17.81C4.54 18.72 5.26 19.46 6.05 20L15.04 14.9L12 13.15L3.88 17.81M11.04 2C10 2.09 9 2.36 8 2.8V13.15L11.04 11.41V2M2 12C2 13.39 2.3 14.77 2.89 16.12L6 14.28V4C3.33 6 2 8.65 2 12Z"


{-| The [`volume-equal`](https://pictogrammers.com/library/mdi/icon/volume-equal/) icon.
-}
volumeEqual : IconShape
volumeEqual =
    iconShape "M3 9H7L12 4V20L7 15H3V9M14 13H22V15H14M14 9H22V11H14Z"


{-| The [`volume-high`](https://pictogrammers.com/library/mdi/icon/volume-high/) icon.
-}
volumeHigh : IconShape
volumeHigh =
    iconShape "M14,3.23V5.29C16.89,6.15 19,8.83 19,12C19,15.17 16.89,17.84 14,18.7V20.77C18,19.86 21,16.28 21,12C21,7.72 18,4.14 14,3.23M16.5,12C16.5,10.23 15.5,8.71 14,7.97V16C15.5,15.29 16.5,13.76 16.5,12M3,9V15H7L12,20V4L7,9H3Z"


{-| The [`volume-low`](https://pictogrammers.com/library/mdi/icon/volume-low/) icon.
-}
volumeLow : IconShape
volumeLow =
    iconShape "M7,9V15H11L16,20V4L11,9H7Z"


{-| The [`volume-medium`](https://pictogrammers.com/library/mdi/icon/volume-medium/) icon.
-}
volumeMedium : IconShape
volumeMedium =
    iconShape "M5,9V15H9L14,20V4L9,9M18.5,12C18.5,10.23 17.5,8.71 16,7.97V16C17.5,15.29 18.5,13.76 18.5,12Z"


{-| The [`volume-mute`](https://pictogrammers.com/library/mdi/icon/volume-mute/) icon.
-}
volumeMute : IconShape
volumeMute =
    iconShape "M3,9H7L12,4V20L7,15H3V9M16.59,12L14,9.41L15.41,8L18,10.59L20.59,8L22,9.41L19.41,12L22,14.59L20.59,16L18,13.41L15.41,16L14,14.59L16.59,12Z"


{-| The [`volume-minus`](https://pictogrammers.com/library/mdi/icon/volume-minus/) icon.
-}
volumeMinus : IconShape
volumeMinus =
    iconShape "M3,9H7L12,4V20L7,15H3V9M14,11H22V13H14V11Z"


{-| The [`volume-off`](https://pictogrammers.com/library/mdi/icon/volume-off/) icon.
-}
volumeOff : IconShape
volumeOff =
    iconShape "M12,4L9.91,6.09L12,8.18M4.27,3L3,4.27L7.73,9H3V15H7L12,20V13.27L16.25,17.53C15.58,18.04 14.83,18.46 14,18.7V20.77C15.38,20.45 16.63,19.82 17.68,18.96L19.73,21L21,19.73L12,10.73M19,12C19,12.94 18.8,13.82 18.46,14.64L19.97,16.15C20.62,14.91 21,13.5 21,12C21,7.72 18,4.14 14,3.23V5.29C16.89,6.15 19,8.83 19,12M16.5,12C16.5,10.23 15.5,8.71 14,7.97V10.18L16.45,12.63C16.5,12.43 16.5,12.21 16.5,12Z"


{-| The [`volume-plus`](https://pictogrammers.com/library/mdi/icon/volume-plus/) icon.
-}
volumePlus : IconShape
volumePlus =
    iconShape "M3,9H7L12,4V20L7,15H3V9M14,11H17V8H19V11H22V13H19V16H17V13H14V11Z"


{-| The [`volume-source`](https://pictogrammers.com/library/mdi/icon/volume-source/) icon.
-}
volumeSource : IconShape
volumeSource =
    iconShape "M3 9V15H7L12 20V4L7 9H3M16 15H14V9H16V15M20 19H18V5H20V19Z"


{-| The [`volume-variant-off`](https://pictogrammers.com/library/mdi/icon/volume-variant-off/) icon.
-}
volumeVariantOff : IconShape
volumeVariantOff =
    iconShape "M5.64,3.64L21.36,19.36L19.95,20.78L16,16.83V20L11,15H7V9H8.17L4.22,5.05L5.64,3.64M16,4V11.17L12.41,7.58L16,4Z"


{-| The [`volume-vibrate`](https://pictogrammers.com/library/mdi/icon/volume-vibrate/) icon.
-}
volumeVibrate : IconShape
volumeVibrate =
    iconShape "M4 9V15H8L13 20V4L8 9H4M16.55 2.47L15.5 3.53L17.93 6L15 9L17.93 12L15 15L17.93 18L15.5 20.47L16.55 21.53L20 18L17.07 15L20 12L17.07 9L20 6L16.55 2.47Z"


{-| The [`vote`](https://pictogrammers.com/library/mdi/icon/vote/) icon.
-}
vote : IconShape
vote =
    iconShape "M18,13H17.32L15.32,15H17.23L19,17H5L6.78,15H8.83L6.83,13H6L3,16V20A2,2 0 0,0 5,22H19A2,2 0 0,0 21,20V16L18,13M17,7.95L12.05,12.9L8.5,9.36L13.46,4.41L17,7.95M12.76,2.29L6.39,8.66C6,9.05 6,9.68 6.39,10.07L11.34,15C11.73,15.41 12.36,15.41 12.75,15L19.11,8.66C19.5,8.27 19.5,7.64 19.11,7.25L14.16,2.3C13.78,1.9 13.15,1.9 12.76,2.29Z"


{-| The [`vote-outline`](https://pictogrammers.com/library/mdi/icon/vote-outline/) icon.
-}
voteOutline : IconShape
voteOutline =
    iconShape "M18,13L21,16V20C21,21.11 20.1,22 19,22H5C3.89,22 3,21.1 3,20V16L6,13H6.83L8.83,15H6.78L5,17H19L17.23,15H15.32L17.32,13H18M19,20V19H5V20H19M11.34,15L6.39,10.07C6,9.68 6,9.05 6.39,8.66L12.76,2.29C13.15,1.9 13.78,1.9 14.16,2.3L19.11,7.25C19.5,7.64 19.5,8.27 19.11,8.66L12.75,15C12.36,15.41 11.73,15.41 11.34,15M13.46,4.41L8.5,9.36L12.05,12.9L17,7.95L13.46,4.41Z"


{-| The [`vpn`](https://pictogrammers.com/library/mdi/icon/vpn/) icon.
-}
vpn : IconShape
vpn =
    iconShape "M9,5H15L12,8L9,5M10.5,14.66C10.2,15 10,15.5 10,16A2,2 0 0,0 12,18A2,2 0 0,0 14,16C14,15.45 13.78,14.95 13.41,14.59L14.83,13.17C15.55,13.9 16,14.9 16,16A4,4 0 0,1 12,20A4,4 0 0,1 8,16C8,14.93 8.42,13.96 9.1,13.25L9.09,13.24L16.17,6.17V6.17C16.89,5.45 17.89,5 19,5A4,4 0 0,1 23,9A4,4 0 0,1 19,13C17.9,13 16.9,12.55 16.17,11.83L17.59,10.41C17.95,10.78 18.45,11 19,11A2,2 0 0,0 21,9A2,2 0 0,0 19,7C18.45,7 17.95,7.22 17.59,7.59L10.5,14.66M6.41,7.59C6.05,7.22 5.55,7 5,7A2,2 0 0,0 3,9A2,2 0 0,0 5,11C5.55,11 6.05,10.78 6.41,10.41L7.83,11.83C7.1,12.55 6.1,13 5,13A4,4 0 0,1 1,9A4,4 0 0,1 5,5C6.11,5 7.11,5.45 7.83,6.17V6.17L10.59,8.93L9.17,10.35L6.41,7.59Z"


{-| Look up an icon (which has the first letter "v") by its name.

    let accountIcon =
        Material.Icons.Directory.lookup "account"

-}
lookup : String -> Maybe IconShape
lookup name =
    case name of
        "vacuum" ->
            Just vacuum

        "vacuum-outline" ->
            Just vacuumOutline

        "valve" ->
            Just valve

        "valve-closed" ->
            Just valveClosed

        "valve-open" ->
            Just valveOpen

        "van-passenger" ->
            Just vanPassenger

        "van-utility" ->
            Just vanUtility

        "vanish" ->
            Just vanish

        "vanish-quarter" ->
            Just vanishQuarter

        "vanity-light" ->
            Just vanityLight

        "variable" ->
            Just variable

        "variable-box" ->
            Just variableBox

        "vector-arrange-above" ->
            Just vectorArrangeAbove

        "vector-arrange-below" ->
            Just vectorArrangeBelow

        "vector-bezier" ->
            Just vectorBezier

        "vector-circle" ->
            Just vectorCircle

        "vector-circle-variant" ->
            Just vectorCircleVariant

        "vector-combine" ->
            Just vectorCombine

        "vector-curve" ->
            Just vectorCurve

        "vector-difference" ->
            Just vectorDifference

        "vector-difference-ab" ->
            Just vectorDifferenceAb

        "vector-difference-ba" ->
            Just vectorDifferenceBa

        "vector-ellipse" ->
            Just vectorEllipse

        "vector-intersection" ->
            Just vectorIntersection

        "vector-line" ->
            Just vectorLine

        "vector-link" ->
            Just vectorLink

        "vector-point" ->
            Just vectorPoint

        "vector-point-edit" ->
            Just vectorPointEdit

        "vector-point-minus" ->
            Just vectorPointMinus

        "vector-point-plus" ->
            Just vectorPointPlus

        "vector-point-select" ->
            Just vectorPointSelect

        "vector-polygon" ->
            Just vectorPolygon

        "vector-polygon-variant" ->
            Just vectorPolygonVariant

        "vector-polyline" ->
            Just vectorPolyline

        "vector-polyline-edit" ->
            Just vectorPolylineEdit

        "vector-polyline-minus" ->
            Just vectorPolylineMinus

        "vector-polyline-plus" ->
            Just vectorPolylinePlus

        "vector-polyline-remove" ->
            Just vectorPolylineRemove

        "vector-radius" ->
            Just vectorRadius

        "vector-rectangle" ->
            Just vectorRectangle

        "vector-selection" ->
            Just vectorSelection

        "vector-square" ->
            Just vectorSquare

        "vector-square-close" ->
            Just vectorSquareClose

        "vector-square-edit" ->
            Just vectorSquareEdit

        "vector-square-minus" ->
            Just vectorSquareMinus

        "vector-square-open" ->
            Just vectorSquareOpen

        "vector-square-plus" ->
            Just vectorSquarePlus

        "vector-square-remove" ->
            Just vectorSquareRemove

        "vector-triangle" ->
            Just vectorTriangle

        "vector-union" ->
            Just vectorUnion

        "vhs" ->
            Just vhs

        "vibrate" ->
            Just vibrate

        "vibrate-off" ->
            Just vibrateOff

        "video" ->
            Just video

        "video-2d" ->
            Just video2d

        "video-3d" ->
            Just video3d

        "video-3d-off" ->
            Just video3dOff

        "video-3d-variant" ->
            Just video3dVariant

        "video-4k-box" ->
            Just video4kBox

        "video-account" ->
            Just videoAccount

        "video-box" ->
            Just videoBox

        "video-box-off" ->
            Just videoBoxOff

        "video-check" ->
            Just videoCheck

        "video-check-outline" ->
            Just videoCheckOutline

        "video-high-definition" ->
            Just videoHighDefinition

        "video-image" ->
            Just videoImage

        "video-input-antenna" ->
            Just videoInputAntenna

        "video-input-component" ->
            Just videoInputComponent

        "video-input-hdmi" ->
            Just videoInputHdmi

        "video-input-scart" ->
            Just videoInputScart

        "video-input-svideo" ->
            Just videoInputSvideo

        "video-marker" ->
            Just videoMarker

        "video-marker-outline" ->
            Just videoMarkerOutline

        "video-minus" ->
            Just videoMinus

        "video-minus-outline" ->
            Just videoMinusOutline

        "video-off" ->
            Just videoOff

        "video-off-outline" ->
            Just videoOffOutline

        "video-outline" ->
            Just videoOutline

        "video-plus" ->
            Just videoPlus

        "video-plus-outline" ->
            Just videoPlusOutline

        "video-stabilization" ->
            Just videoStabilization

        "video-standard-definition" ->
            Just videoStandardDefinition

        "video-switch" ->
            Just videoSwitch

        "video-switch-outline" ->
            Just videoSwitchOutline

        "video-vintage" ->
            Just videoVintage

        "video-wireless" ->
            Just videoWireless

        "video-wireless-outline" ->
            Just videoWirelessOutline

        "view-agenda" ->
            Just viewAgenda

        "view-agenda-outline" ->
            Just viewAgendaOutline

        "view-array" ->
            Just viewArray

        "view-array-outline" ->
            Just viewArrayOutline

        "view-carousel" ->
            Just viewCarousel

        "view-carousel-outline" ->
            Just viewCarouselOutline

        "view-column" ->
            Just viewColumn

        "view-column-outline" ->
            Just viewColumnOutline

        "view-comfy" ->
            Just viewComfy

        "view-comfy-outline" ->
            Just viewComfyOutline

        "view-compact" ->
            Just viewCompact

        "view-compact-outline" ->
            Just viewCompactOutline

        "view-dashboard" ->
            Just viewDashboard

        "view-dashboard-edit" ->
            Just viewDashboardEdit

        "view-dashboard-edit-outline" ->
            Just viewDashboardEditOutline

        "view-dashboard-outline" ->
            Just viewDashboardOutline

        "view-dashboard-variant" ->
            Just viewDashboardVariant

        "view-dashboard-variant-outline" ->
            Just viewDashboardVariantOutline

        "view-day" ->
            Just viewDay

        "view-day-outline" ->
            Just viewDayOutline

        "view-gallery" ->
            Just viewGallery

        "view-gallery-outline" ->
            Just viewGalleryOutline

        "view-grid" ->
            Just viewGrid

        "view-grid-compact" ->
            Just viewGridCompact

        "view-grid-outline" ->
            Just viewGridOutline

        "view-grid-plus" ->
            Just viewGridPlus

        "view-grid-plus-outline" ->
            Just viewGridPlusOutline

        "view-headline" ->
            Just viewHeadline

        "view-list" ->
            Just viewList

        "view-list-outline" ->
            Just viewListOutline

        "view-module" ->
            Just viewModule

        "view-module-outline" ->
            Just viewModuleOutline

        "view-parallel" ->
            Just viewParallel

        "view-parallel-outline" ->
            Just viewParallelOutline

        "view-quilt" ->
            Just viewQuilt

        "view-quilt-outline" ->
            Just viewQuiltOutline

        "view-sequential" ->
            Just viewSequential

        "view-sequential-outline" ->
            Just viewSequentialOutline

        "view-split-horizontal" ->
            Just viewSplitHorizontal

        "view-split-vertical" ->
            Just viewSplitVertical

        "view-stream" ->
            Just viewStream

        "view-stream-outline" ->
            Just viewStreamOutline

        "view-week" ->
            Just viewWeek

        "view-week-outline" ->
            Just viewWeekOutline

        "violin" ->
            Just violin

        "virtual-reality" ->
            Just virtualReality

        "virus" ->
            Just virus

        "virus-off" ->
            Just virusOff

        "virus-off-outline" ->
            Just virusOffOutline

        "virus-outline" ->
            Just virusOutline

        "voicemail" ->
            Just voicemail

        "volcano" ->
            Just volcano

        "volcano-outline" ->
            Just volcanoOutline

        "volleyball" ->
            Just volleyball

        "volume-equal" ->
            Just volumeEqual

        "volume-high" ->
            Just volumeHigh

        "volume-low" ->
            Just volumeLow

        "volume-medium" ->
            Just volumeMedium

        "volume-mute" ->
            Just volumeMute

        "volume-minus" ->
            Just volumeMinus

        "volume-off" ->
            Just volumeOff

        "volume-plus" ->
            Just volumePlus

        "volume-source" ->
            Just volumeSource

        "volume-variant-off" ->
            Just volumeVariantOff

        "volume-vibrate" ->
            Just volumeVibrate

        "vote" ->
            Just vote

        "vote-outline" ->
            Just voteOutline

        "vpn" ->
            Just vpn

        _ ->
            Nothing
